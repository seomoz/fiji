...
public final class BentoCluster {
  ...
  public void start(
    ...
    if (pypiRepository.isPresent()) {
      commandsBuilder.add(
          String.format("env python3 -m pip install fiji-bento-cluster -i %s", pypiRepository.get())
      );
    } else {
      commandsBuilder.add("env python3 -m pip install fiji-bento-cluster");
    }
