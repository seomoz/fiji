...
class FijiCommand(command.Command):
  """Shell command assuming a FijiBento environment."""

  def __init__(self, command, **kwargs):
    """Runs a Fiji command line.

    Requires the current working directory to be the FijiBento installation dir.
    ...
    args = [
        '/bin/bash',
        '-c',
        'source ./bin/fiji-env.sh --override > /dev/null 2>&1 && %s' % command,
    ]
    super(FijiCommand, self).__init__(*args, **kwargs)
