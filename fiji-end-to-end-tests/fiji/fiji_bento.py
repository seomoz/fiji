...
from fiji import bento_cluster
from fiji import maven_repo
...
class FijiBento(object):
  """FijiBento distribution.

  Wraps a FijiBento installation.
  """

  def __init__(self, path, version=None):
    """Initializes the FijiBento object.

    Args:
      path: Path of the FijiBento install directory.
  ...
  @property
  def installed(self):
    return os.path.exists(os.path.join(self.path, 'bin', 'fiji-env.sh'))
  ...
  def Install(self):
    """Ensures FijiBento is installed properly."""
  ...
  def _Fetch(self, version):
    ...
    # Strip the first path component from the fiji-bento release archive:
    # The top-level directory is "fiji-bento-<code-name>/",
...
class CLI(cli.Action):
  def RegisterFlags(self):
    self.flags.AddString(
        name='install_dir',
        default='/tmp/fiji-bento',
        help='Path where FijiBento is installed.',
    )
    self.flags.AddString(
        name='version',
        default='1.0.0',
        help='FijiBento version.',
    )
    self.flags.AddString(
        name='do',
        default='install',
