 ...
 * <tt>fiji-delegation.properties</tt> to find the correct implementation to use
 * for a given interface.</p>
 *
 * <p>The <tt>fiji-delegation.properties</tt> file is a Java properties file
 ...
 * specified through <tt>META-INF/services/</tt>. The <tt>fiji-delegation.properties</tt>
...
public final class ConfiguredLookup<T> extends Lookup<T> {
  ...
  public static final String CONF_RESOURCE_NAME = "fiji-delegation.properties";

  /**
   * A cache of parsed fiji-delegation.properties files, maintained on a per-ClassLoader basis.
   ...
   * a new instance. This first looks to the property line in the fiji-delegation.properties
  ...
  private Properties getPropertiesForClassLoader() {
    ...
    // Walk through the available fiji-delegation.properties files and parse them.
    Properties confProps = new Properties();
    try {
      Enumeration<URL> resources = mClassLoader.getResources(CONF_RESOURCE_NAME);
      if (null == resources) {
        LOG.debug("No fiji-delegation.properties file was found on the classpath, but interface");
        ...
        try {
          ...
          for (Map.Entry<Object, Object> resourceEntry : resourceProps.entrySet()) {
            ...
            if (!confProps.containsKey(key)) {
              ...
              LOG.debug("Multiple copies of fiji-delegation.properties on the classpath define");
              LOG.debug("a binding for interface " + mClass.getName());
              LOG.debug("Using implementation: " + confProps.get(key));
            }
          }
        } catch (IOException ioe) {
          LOG.error("Could not load fiji-delegation configuration data from classpath entry: "
