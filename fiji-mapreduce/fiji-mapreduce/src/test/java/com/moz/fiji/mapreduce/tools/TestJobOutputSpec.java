...
import com.moz.fiji.mapreduce.tools.framework.JobIOSpecParseException;
import com.moz.fiji.mapreduce.tools.framework.JobOutputSpec;
...
public class TestJobOutputSpec {
  ...
  @Test
  public void testParseFijiColon() {
    try {
      JobOutputSpec.parse("fiji:");
      fail("Should have gotten a JobIOSpecParseException.");
    } catch (JobIOSpecParseException jpe) {
      assertEquals("Invalid job output spec, expecting 'format:location@nsplit'. [spec=fiji:]",
          jpe.getMessage());
    }
  }

  @Test
  public void testParseFijiNoSplits() {
    try {
      JobOutputSpec.parse("fiji:fiji://hbase/instance/table");
      fail("Should have gotten a JobIOSpecParseException.");
    } catch (JobIOSpecParseException jpe) {
      assertEquals("Invalid job output spec, expecting 'format:location@nsplit'. "
          + "[spec=fiji:fiji://hbase/instance/table]",
          jpe.getMessage());
    }
  }
  ...
  @Test
  public void testParseHFileHDFSWithFiji() {
    JobOutputSpec spec =
        JobOutputSpec.parse("hfile:fiji://hbase/instance/table;hdfs://localhost:1234/tmp/foo@8");
    assertEquals(JobOutputSpec.Format.HFILE, spec.getFormat());
    assertEquals("fiji://hbase/instance/table;hdfs://localhost:1234/tmp/foo", spec.getLocation());
    assertEquals(8, spec.getSplits());
  }
