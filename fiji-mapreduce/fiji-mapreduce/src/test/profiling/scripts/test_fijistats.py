...
'''
test_fijistats.py is a script to unit test the fijistats.py script to ensure it is
performing the expected computations, such as aggregating based on jobname or function name.
It uses pre-existing files in fiji-mapreduce/src/test/profiling/resources/ that
...
arch -i386 /usr/bin/python test_fijistats.py
...
import sys
import unittest
# path to where the fijistats script is located
srcpath = os.path.abspath(os.getcwd() + '/../../../main/profiling/scripts')
sys.path.insert(0, srcpath)
import fijistats
import tempfile

# Unit test for the fijistats.py script
class TestFijiStats(unittest.TestCase):
    def setUp(self):
    ...
    def testArgsForJob(self):
        ...
        self.assertRaises(ValueError, fijistats.main, ['fijistats', '--to-file', 'xyz'])
    ...
    def testByJobName(self):
        expectedOutput = "public synchronized com.moz.fiji.schema.FijiSchemaTable.SchemaEntry " + \
            "com.moz.fiji.schema.impl.HBaseSchemaTable.getSchemaEntry(com.moz.fiji.schema.util.BytesKey)," + \
            " 278514701, 875, 709618.05"
        tf = tempfile.NamedTemporaryFile(delete=False)
        filename = tf.name
        fijistats.main(['./fijistats.py',
                        '--stats-dir',
                        self.input_dir,
                        '--by-jobname',
                        'TestingGatherer',
...
suite = unittest.TestLoader().loadTestsFromTestCase(TestFijiStats)
