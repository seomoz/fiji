...
/**
 * Factory for a specific FijiMRPlatformBridge implementation. This class represents
 * the service loaded by the delegation library; we do not directly load multiple
 * FijiMRPlatformBridge implementations, because they may not all typecheck against
 * a particular live runtime environment (e.g., a class/interface different).
 *
 * <p>Individual FijiMRPlatformBridgeFactory implementations should use Class.forName()
 * to load a specific FijiMRPlatformBridge implementation dynamically only after it
 * has been chosen by the PriorityLookup library as the best implementation fit.</p>
 *
 * <p>It is very important that FijiMRPlatformBridgeFactory does not inadvertently
 * attempt to classload portions of the Hadoop or HBase runtime as this may trigger
 * typechecking by the JVM that fails. FijiMRPlatformBridgeFactory implementations
...
abstract class FijiMRPlatformBridgeFactory implements PriorityProvider {
   ...
   */
  public abstract FijiMRPlatformBridge getBridge();
}
