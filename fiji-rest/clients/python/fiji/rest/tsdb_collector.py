...
from fiji.rest import tsdb_client
...
def ListMetrics(values, path=None):
  """Lists the metrics that can be populated from a FijiREST metrics report.

  Args:
    values: FijiREST metrics report (JSON decoded object).
...
class TSDBCollector(cli.Action):
  """TSDB collector that pushes FijiREST metrics."""
  ...
  def RegisterFlags(self):
    ...
    self.flags.AddString(
        name='fiji_rest_address',
        default='localhost:8000',
        help='host:port address of the FijiREST main interface.',
    )
    self.flags.AddString(
        name='fiji_rest_admin_address',
        default=None,
        help=('host:port address of the FijiREST admin interface.\n'
  ...
  @property
  def hostname(self):
    """Returns: the hostname of the FijiREST server to report metrics of."""
    return self._hostname

  def Run(self, args):
    assert (len(args) == 0), ('Extra arguments: %r' % args)

    if self.flags.fiji_rest_admin_address is None:
      (host, port) = self.flags.fiji_rest_address.split(':')
      port = int(port)
      self.flags.fiji_rest_admin_address = '%s:%d' % (host, port + 1)
    ...
    self._rest_client = fiji_rest.FijiRestClient(
      address = self.flags.fiji_rest_address,
      admin_address = self.flags.fiji_rest_admin_address,
    )

    while True:
      logging.info(
          'Starting FijiREST metrics collection loop for host %s',
          self.hostname)
      self._CollectLoop()

  def _CollectLoop(self):
    """Internal loop that collects FijiREST metrics and pushes them to TSDB."""
    ...
    try:
      while True:
        ...
        try:
          ...
          logging.error(
              'Error while pushing FijiREST metrics for hostname=%s timestamp=%s',
  ...
  def _CollectAndPush(self, timestamp):
    ...
    """
    logging.debug(
        'Collecting metrics from FijiREST server %r (admin %r)',
        self._rest_client.address, self._rest_client.admin_address)
    metrics = self._rest_client.GetMetrics()
    logging.debug('Pushing FijiREST metrics: %r', metrics)
    Push(
        tsdb = self._tsdb,
