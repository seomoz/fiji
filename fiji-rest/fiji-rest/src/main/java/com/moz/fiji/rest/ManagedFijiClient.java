...
import com.moz.fiji.rest.util.FijiInstanceCache;
...
/**
 * Managed resource for tracking Fiji connections.
 */
public class ManagedFijiClient implements FijiClient, Managed {
  private static final Logger LOG = LoggerFactory.getLogger(ManagedFijiClient.class);

  public static final long DEFAULT_TIMEOUT = 10;

  /** Holds instances currently being served. */
  private final LoadingCache<String, FijiInstanceCache> mInstanceCaches;

  /** CuratorFramework object which is backing <code>mZKInstances</code>. */
  private final CuratorFramework mZKFramework;

  /**
   * A 'cache' object which keeps track of the currently registered Fiji instances in ZooKeeper.
   */
  private final PathChildrenCache mZKInstances;

  /**
   * Holds the currently known Fiji instances as registered in ZooKeeper. It is sufficient for this
   ...
   */
  private volatile Set<String> mFijiInstances;
  private final Set<String> mVisibleFijiInstances;

  /** Tracks the lifecycle state of this ManagedFijiClient. */
  private final AtomicReference<State> mState;

  /** The possible states of this ManagedFijiClient. */
  private static enum State {
    /** ManagedFijiClient is constructed, but not yet started. */
    INITIALIZED,
    /** ManagedFijiClient is started and alive. */
    STARTED,
    /** ManagedFijiClient is stopped. */
    STOPPED
  }

  /**
   * Constructs a ManagedFijiClient.
   *
   * @param configuration of HBase cluster to serve.
   * @throws IOException if error while creating connections to the cluster.
   */
  public ManagedFijiClient(final FijiRESTConfiguration configuration) throws IOException {
    this(FijiURI.newBuilder(configuration.getClusterURI()).build(),
   ...
   * @throws IOException if error while creating connections to the cluster.
   */
  public ManagedFijiClient(final FijiURI clusterURI) throws IOException {
   ...
   * @throws IOException if error while creating connections to the cluster.
   */
  public ManagedFijiClient(final FijiURI clusterURI,
    ...
    mInstanceCaches = CacheBuilder
        ...
        .removalListener(new RemovalListener<String, FijiInstanceCache>() {
          @Override
          public void onRemoval(RemovalNotification<String, FijiInstanceCache> notification) {
            try {
              notification.getValue().stop(); // strong cache; should not be null
            } catch (IOException e) {
              LOG.warn("Unable to stop FijiInstanceCache {} for instance {}.",
                  notification.getValue(), notification.getKey());
            }
          }
        })
        .build(new CacheLoader<String, FijiInstanceCache>() {
          @Override
          public FijiInstanceCache load(String instanceName) throws Exception {
            final FijiURI instanceURI =
                FijiURI.newBuilder(clusterURI).withInstanceName(instanceName).build();

            // Check if our instances list contains the instance before attempting to construct it.
            if (!mFijiInstances.contains(instanceName)) {
              throw new FijiNotInstalledException(
                  "Fiji instance not found in known instances set.", instanceURI);
            }
            return new FijiInstanceCache(instanceURI);
          }
        });
  ...
  public void start() throws Exception {
    Preconditions.checkState(mState.compareAndSet(State.INITIALIZED, State.STARTED),
        "Can not start ManagedFijiClient in state %s.", mState.get());
    ...
    refreshInstances();
    LOG.info("Successfully started ManagedFijiClient!");
  }
   ...
   */
  private FijiInstanceCache getInstanceCache(String instance) {
    try {
  ...
  private Response.Status getExceptionStatus(Throwable cause) {
    if (cause instanceof FijiNotInstalledException) {
      return Response.Status.FORBIDDEN;
    }
    if (cause instanceof FijiTableNotFoundException) {
  ...
  /** {@inheritDoc} */
  @Override
  public Fiji getFiji(String instance) {
    final State state = mState.get();
    Preconditions.checkState(state == State.STARTED,
        "Can not get a Fiji while in state %s.", state);
    return getInstanceCache(instance).getFiji();
  }

  /** {@inheritDoc} */
  @Override
  public FijiSchemaTable getFijiSchemaTable(String instance) {
    final State state = mState.get();
    Preconditions.checkState(state == State.STARTED,
        "Can not get a schema table while in state %s.", state);
    try {
      return getFiji(instance).getSchemaTable();
    } catch (IOException e) {
  ...
  public Collection<String> getInstances() {
    ...
    return mFijiInstances;
  }

  /** {@inheritDoc} */
  @Override
  public FijiTable getFijiTable(String instance, String table) {
    final State state = mState.get();
    Preconditions.checkState(state == State.STARTED,
        "Can not get Fiji table while in state %s.", state);
    try {
      return getInstanceCache(instance).getFijiTable(table);
  ...
  /** {@inheritDoc} */
  @Override
  public FijiTableReader getFijiTableReader(String instance, String table) {
    final State state = mState.get();
    Preconditions.checkState(state == State.STARTED,
        "Can not get fresh Fiji table reader while in state %s.", state);
    try {
      return getInstanceCache(instance).getFijiTableReader(table);
    } catch (ExecutionException e) {
      final Throwable cause = e.getCause();
      throw new WebApplicationException(cause, getExceptionStatus(cause));
    } catch (WebApplicationException e) {
  ...
  /**
   * Update the instances served by this ManagedFijiClient.
   *
  ...
  public void refreshInstances() throws IOException {
    ...
    // by FijiREST keeping only the ones that were specified in the config.
    if (mVisibleFijiInstances == null || !mVisibleFijiInstances.isEmpty()) {
      for (ChildData node : mZKInstances.getCurrentData()) {
        instances.add(Iterables.getLast(Splitter.on('/').split(node.getPath())));
      }

      if (mVisibleFijiInstances != null) {
        // Keep the intersection of the visible and actual sets.
        instances.retainAll(mVisibleFijiInstances);
      }
    }
    final ImmutableSet.Builder<String> instancesBuilder = ImmutableSet.builder();
    instancesBuilder.addAll(instances);
    mFijiInstances = instancesBuilder.build();
  }

  /**
   * Check whether this FijiClient is healthy.
   *
   * @return health status of the FijiClient.
   */
  public HealthCheck.Result checkHealth() {
    ...
    for (FijiInstanceCache instanceCache : mInstanceCaches.asMap().values()) {
  ...
  /**
   * A {@link PathChildrenCacheListener} to listen to the Fiji instances ZNode directory and
   * update the set of available Fiji instances.
   */
  private class InstanceListener implements PathChildrenCacheListener {
    /**
     * Creates a new InstanceListener to update the parent ManagedFijiClient.
     */
    ...
    public void childEvent(
      ...
      LOG.debug("InstanceListener for {} triggered on event {}.", ManagedFijiClient.this, event);
      refreshInstances();
    }
  }
}
