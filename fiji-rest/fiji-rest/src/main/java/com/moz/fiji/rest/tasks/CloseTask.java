...
/**
 * This REST task allows administrators to close open Fiji instances and tables so that
...
public class CloseTask extends Task {
  ...
  /**
   * Create a CloseTask with the provided FijiClient.
   *
   * @param fijiClient to use to close instances and tables.
   */
  public CloseTask(FijiClient fijiClient) {
    super("close");
  ...
  public void execute(
    ...
    if (tables.isEmpty()) {
      // Close the instance
      mFijiClient.invalidateInstance(instance);
    } else {
