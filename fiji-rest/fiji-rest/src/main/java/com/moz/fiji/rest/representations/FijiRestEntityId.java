 ...
 */
public final class FijiRestEntityId {
  ...
  /**
   * Private constructor for FijiRestEntityId parametrized by a String.
   * Validate fields as necessary.
   *
   * @param stringEntityId string representing the entity id.
   */
  private FijiRestEntityId(
  ...
  /**
   * Private constructor for FijiRestEntityId parametrized by an array of components.
   * Validate fields as necessary.
   *
  ...
  /**
   * Create FijiRestEntityId from a string input, which can be a raw hbase rowKey prefixed by
   * 'hbase=' or 'hbase_hex=' (the former for bytes-encoding and the latter for ASCII encoding).
   *
   * @param entityId string of the row.
   * @return a properly constructed FijiRestEntityId.
   * @throws IOException if FijiRestEntityId can not be properly constructed.
   */
  public static FijiRestEntityId create(
      final String entityId) throws IOException {
    return new FijiRestEntityId(entityId);
  }

  /**
   * Create FijiRestEntityId from a string input, which can be a json string or a raw hbase rowKey.
   ...
   * @throws IOException if FijiRestEntityId can not be properly constructed.
   */
  public static FijiRestEntityId createFromUrl(
      final String entityId,
      final FijiTableLayout layout) throws IOException {
  ...
  /**
   * Create a list of FijiRestEntityIds from a string input, which can be a json array of valid
   ...
   * @param layout of the table in which the entity id belongs.
   *        If null, then long components may not be recognized.
   * @return a properly constructed list of FijiRestEntityIds.
   * @throws IOException if FijiRestEntityId list can not be properly constructed.
   */
  public static List<FijiRestEntityId> createListFromUrl(
    ...
    if (jsonNode.isArray()) {
      for (JsonNode node : jsonNode) {
        if (node.isTextual()) {
          fijiRestEntityIds.add(createFromUrl(node.textValue(), layout));
        } else {
          fijiRestEntityIds.add(createFromUrl(node.toString(), layout));
        }
      }
    } else {
    ...
    return fijiRestEntityIds;
  }

  /**
   * Create FijiRestEntityId from entity id and layout.
   *
  ...
  public static FijiRestEntityId create(
    ...
    case FORMATTED:
      ...
      if (!hashSpec.getSuppressKeyMaterialization()) {
        return new FijiRestEntityId(entityId.getComponents().toArray(), false);
      } else {
        return new FijiRestEntityId(
  ...
  /**
   * Create FijiRestEntityId from json node.
   *
   * @param node of the RKF2-formatted, materialization unsuppressed row.
   * @return a properly constructed FijiRestEntityId.
   * @throws IOException if FijiRestEntityId can not be properly constructed.
   */
  public static FijiRestEntityId create(final JsonNode node) throws IOException {
   ...
   */
  private static RowKeyFormat2 getRKF2(final FijiTableLayout layout) throws IOException {
    if (null != layout
  ...
  public static FijiRestEntityId create(
    ...
    if (node.isArray()) {
      ...
      return new FijiRestEntityId(components, wildCarded);
    } else {
      // Disallow non-arrays.
   ...
   */
  public EntityId resolve(final FijiTableLayout layout) throws IOException {
