...
import com.moz.fiji.rest.sample_avro.PickBan;
import com.moz.fiji.rest.sample_avro.Team;
...
public class TestRowsResource extends ResourceTest {
  ...
  private Fiji mFakeFiji = null;
  private FijiSchemaTable mSchemaTable = null;
  private ManagedFijiClient mFijiClient = null;
  ...
  protected void setUpResources() throws Exception {
    ...
    mFakeFiji = builder.build();

    mSchemaTable = mFakeFiji.getSchemaTable();

    mStringOption = new SchemaOption(Schema.create(Type.STRING).toString());

    Set<FijiURI> mValidInstances = Sets.newHashSet();

    TableLayoutDesc desc = FijiTableLayouts.getLayout("com.moz.fiji/rest/layouts/sample_table.json");

    mFakeFiji.createTable(desc);
    mValidInstances.add(mFakeFiji.getURI());

    // Add some data
    FijiTable fakeTable = mFakeFiji.openTable("sample_table");
    EntityId eid = fakeTable.getEntityId(12345L);
    FijiTableWriter writer = fakeTable.openTableWriter();
    ...
    CellSpec spec = fakeTable.getLayout().getCellSpec(
        FijiColumnName.create("group_family", "inline_record"));
    ...
    final FijiTableLayout playersTableLayout =
        FijiTableLayouts.getTableLayout("com.moz.fiji/rest/layouts/players_table.json");
    mFakeFiji.createTable(playersTableLayout.getDesc());
    final FijiTable playersTable = mFakeFiji.openTable("players");
    final FijiTableWriter playersTableWriter = playersTable.openTableWriter();
    ...
    mFijiClient = new ManagedFijiClient(mFakeFiji.getURI());
    mFijiClient.start();

    RowsResource resource = new RowsResource(mFijiClient, getObjectMapper());
    addResource(resource);
  }

  protected final String getHBaseRowKeyHex(String table, Object... components) throws IOException {
    FijiTable fakeTable = mFakeFiji.openTable(table);
  ...
  protected final String createJsonArray(String... components) throws IOException {
    ...
    for (String component : components) {
      JsonNode node;
      if (component.startsWith(FijiRestEntityId.HBASE_ROW_KEY_PREFIX)
          || component.startsWith(FijiRestEntityId.HBASE_HEX_ROW_KEY_PREFIX)) {
  ...
  public void testShouldFetchAllCellsForGivenRow() throws Exception {
    ...
    URI resourceURI = UriBuilder
        ...
        .queryParam("eid", eid)
        .build("default", "sample_table");
    FijiRestRow returnRow = client().resource(resourceURI).get(FijiRestRow.class);
  ...
  public void  testShouldFailOnMalformedEids() throws Exception {
    ...
    URI resourceURI = UriBuilder.fromResource(RowsResource.class)
        ...
        .build("default", "sample_table");
    try {
      client().resource(resourceURI).get(FijiRestRow.class);
  ...
  public void testShouldReturnRowsInRange() throws Exception {
    ...
    FijiRestRow row = client().resource(resourceURI).get(FijiRestRow.class);
  ...
  public void testSingleCellPost() throws Exception {
    ...
    FijiCell<String> postCell = fromInputs("group_family", "string_qualifier", 314592L,
        "helloworld", Schema.create(Type.STRING));

    FijiRestRow postRow = new FijiRestRow(
        FijiRestEntityId.create(stringToJsonNode(
  ...
  public void testCounterPost() throws Exception {
    // Set up.
    FijiCell<Long> postCell = fromInputs("info",
        "logins",
        0L,
        0L,
    ...
    FijiRestRow postRow = new FijiRestRow(FijiRestEntityId.create(stringToJsonNode(eid)));
    ...
    // Increment counter.
    FijiRestRow postRowIncrement = new FijiRestRow(FijiRestEntityId.create(stringToJsonNode(eid)));
    ...
    resourceURI = UriBuilder.fromResource(RowsResource.class)
        .queryParam("eid", eid)
        .build("default", "players");
    returnRow = client().resource(resourceURI).get(FijiRestRow.class);
    ...
    // Decrement counter.
    FijiRestRow postRowDecrement = new FijiRestRow(FijiRestEntityId.create(stringToJsonNode(eid)));
  ...
  public void testShouldPostUnionType() throws Exception {
    ...
    FijiCell<String> postCell = fromInputs("group_family", "union_qualifier", 314592L,
        "helloworld", Schema.create(Type.STRING));
  ...
  public void testMultipleCellPost() throws Exception {
    ...
    FijiCell<String> postCell1 = fromInputs("group_family", "string_qualifier", 314592L,
        "helloworld", Schema.create(Type.STRING));
    FijiCell<Long> postCell2 = fromInputs("group_family", "long_qualifier", 314592L, 123L,
        Schema.create(Type.LONG));
    ...
    FijiCell<JsonNode> postCell3 = fromInputs("group_family", "team_qualifier", 314592L,
  ...
  public void testGenericAvroPost() throws Exception {
    ...
    FijiTableLayout layout = FijiTableLayouts
        .getTableLayout("com.moz.fiji/rest/layouts/sample_table.json");
    CellSpec spec = layout.getCellSpec(FijiColumnName.create("group_family", "inline_record"));
    ...
    FijiCell<JsonNode> postCell = fromInputs("group_family", "inline_record", 3141592L,
  ...
  public void testMapColumnPost() throws Exception {
    ...
    FijiCell<String> postCell1 = fromInputs("strings", EXTENSIVE_COLUMN_TEST,
        3141592L, "helloworld", Schema.create(Type.STRING));
    String longsColumnDec = " ";

    FijiCell<Long> postCell2 = fromInputs("longs", longsColumnDec, 3141592L, 987654567890L,
        Schema.create(Type.LONG));
    ...
    FijiCell<JsonNode> postCell3 = fromInputs("pick_bans", bansColumnDec, 3141592L,
  ...
  public void testNonexistentColumnPost() throws Exception {
    ...
    FijiCell<String> postCell = fromInputs("nonfamily", "noncolumn", 314592L, "hagar",
  ...
  public void testTimestampedPost() throws Exception {
    ...
    FijiCell<String> postCell1 = fromInputs("strings", EXTENSIVE_COLUMN_TEST, 2L,
        "sample_string", Schema.create(Type.STRING));
    FijiCell<Long> postCell2 = fromInputs("group_family", "long_qualifier", 3141591L, 123L,
        Schema.create(Type.LONG));
  ...
  private static <T> FijiCell<T> fromInputs(String family, String qualifier,
  ...
  public void testBatchPost() throws Exception {
    ...
    FijiCell<String> postCell1 = fromInputs("strings", stringsColumn, 2L, "sample_string",
        Schema.create(Type.STRING));
    FijiCell<Integer> postCell2 = fromInputs("group_family", "long_qualifier", 3141591L, 123,
        Schema.create(Type.LONG));
    FijiRestRow postRow = new FijiRestRow(
        FijiRestEntityId.create(stringToJsonNode(
        URLDecoder.decode(stringRowKey, "UTF-8"))));
    ...
    List<FijiRestRow> postRows = Lists.newLinkedList();
    postRows.add(postRow);

    postRow = new FijiRestRow(
        FijiRestEntityId.create(stringToJsonNode(
        URLDecoder.decode(stringRowKey, "UTF-8"))));
    addCellToRow(postRow, postCell2);
    postRows.add(postRow);
  ...
  private void addCellToRow(FijiRestRow rowToModify, FijiCell<?> cellToPost) throws IOException {
