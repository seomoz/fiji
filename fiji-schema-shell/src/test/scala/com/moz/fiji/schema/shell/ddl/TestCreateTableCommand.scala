...
import com.moz.fiji.schema.shell.ddl.key._
...
class TestCreateTableCommand extends CommandTestCase {
  "CreateTableCommand" should {
    ...
    "support a max version less than FijiSchema's max supported version" in {
      // If this check fails, then this tool needs to depend on a newer version of FijiSchema.

      // Defined as TableDDLCommand.MAX_LAYOUT_VERSION
      val ctcmd = new CreateTableCommand(env, "foo", Some("desc"), DefaultKeySpec, List(), Map())
      (ctcmd.MAX_LAYOUT_VERSION.compareTo(FijiTableLayout.getMaxSupportedLayoutVersion())
          must beLessThan(1))
    ...
    "create reasonable looking Avro records" in {
      ...
      FijiTableLayout.getEncoding(layout.getKeysFormat()) mustEqual RowKeyEncoding.FORMATTED
      ...
      // Check that this succeeds. MockFijiSystem will validate that enough
    ...
    "support row format raw" in {
      ...
      val layout = ctcmd.getInitialLayout()
      ctcmd.updateLayout(layout)
      FijiTableLayout.getEncoding(layout.getKeysFormat()) mustEqual RowKeyEncoding.RAW
