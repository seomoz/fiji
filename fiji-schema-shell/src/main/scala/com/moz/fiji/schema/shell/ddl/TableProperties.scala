...
trait TableProperties {
  ...
  final val TableValidationMetaKey = "fiji.shell.column.validation"
  ...
  final val RegionCountMetaKey = "fiji.shell.initial.region.count"
    ...
    tableProperties.foreach { case (k, v) =>
      k match {
        ...
        case InitialRegionCount => {
          ...
          env.fijiSystem.setMeta(env.instanceURI, tableName, RegionCountMetaKey,
              v.asInstanceOf[Int].toString())
        }
        case TableValidationPref => {
          // Save this value for use in subsequent column creation statements.
          env.fijiSystem.setMeta(env.instanceURI, tableName, TableValidationMetaKey,
