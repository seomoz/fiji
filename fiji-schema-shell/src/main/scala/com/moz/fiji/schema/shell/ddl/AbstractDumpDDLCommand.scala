...
trait AbstractDumpDDLCommand extends TableProperties {
  ...
  def dumpLayout(layout: TableLayoutDesc): Unit = {
    ...
    FijiTableLayout.getEncoding(layout.getKeysFormat()) match {
      ...
      case RowKeyEncoding.HASH_PREFIX => {
        val prefixSize = FijiTableLayout.getHashSize(layout.getKeysFormat())
  ...
  def dumpTableProperties(layout: TableLayoutDesc): Unit = {
    val numRegions: String = env.fijiSystem.getMeta(env.instanceURI, layout.getName(),
    ...
    val cellSchemaContext = CellSchemaContext.create(env, TableLayoutDesc.newBuilder(layout))
    if (cellSchemaContext.supportsLayoutValidation()) {
      val validationPolicy: String = env.fijiSystem.getMeta(env.instanceURI, layout.getName(),
      ...
      case SchemaType.AVRO => {
        if (schema.getSpecificReaderSchemaClass() != null) {
          ...
          val avroSchema: Schema =
              env.fijiSystem.getSchemaFor(env.instanceURI, schema.getDefaultReader()).get
  ...
  def dumpAddSchema(tableName: String, cellSchema: CellSchema, familyName: String,
    ...
    readers.foreach { reader: AvroSchema =>
      val schema: Schema = env.fijiSystem.getSchemaFor(env.instanceURI, reader).get
    ...
    writers.foreach { writer: AvroSchema =>
      val schema: Schema = env.fijiSystem.getSchemaFor(env.instanceURI, writer).get
    ...
    Option(cellSchema.getDefaultReader()) match {
      case Some(defaultReader) => {
        val schema: Schema = env.fijiSystem.getSchemaFor(env.instanceURI, defaultReader).get
