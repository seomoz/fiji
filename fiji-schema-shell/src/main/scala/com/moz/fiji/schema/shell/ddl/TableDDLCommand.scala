...
import com.moz.fiji.schema.shell.TableNotFoundException
...
abstract class TableDDLCommand extends DDLCommand {
  ...
  /**
   * Retrieve the table layout to modify from Fiji.
   */
  def getInitialLayout(): TableLayoutDesc.Builder = {
    env.fijiSystem.getTableLayout(getFijiURI(), tableName) match {
      case Some(layout) => {
        val desc = layout.getDesc()
        val tableProtocol = ProtocolVersion.parse(desc.getVersion())

        // Do not modify tables created with newer versions of FijiSchema. We cannot
  ...
  /**
   * Given a built table layout, apply it to the Fiji instance (e.g., by creating a table,
   * or updating an existing one.) The default behavior is to assume the table
   * already exists, and apply the layout to the table using FijiAdmin.
   */
  def applyUpdate(layout: TableLayoutDesc): Unit = {
    ...
    env.fijiSystem.applyLayout(getFijiURI(), tableName, layout)
  }
