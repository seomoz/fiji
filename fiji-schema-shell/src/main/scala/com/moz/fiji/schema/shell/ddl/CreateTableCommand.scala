...
import com.moz.fiji.schema.shell.ddl.key.FormattedKeySpec
...
final class CreateTableCommand(val env: Environment,
  ...
  override def getInitialLayout(): TableLayoutDesc.Builder = {
    val instanceFormat: ProtocolVersion = env.fijiSystem.getSystemVersion(env.instanceURI)
  ...
  override def applyUpdate(layout: TableLayoutDesc): Unit = {
    env.fijiSystem.createTable(getFijiURI(), FijiTableLayout.newLayout(layout),
