...
/**
 * An object used to run a Fiji schema shell.
 */
@ApiAudience.Private
final class ShellMain {
  @Flag(name="fiji", usage="Fiji instance URI")
  var fijiURI: String = "fiji://.env/%s".format(KConstants.DEFAULT_INSTANCE_NAME)
  ...
  def run(): Int = {
    ...
    if (chatty) {
      println("Fiji schema shell v" + ShellMain.version())
    ...
    processUserInput()
    if (chatty) {
      println("Thank you for flying Fiji!")
    }
  ...
  def initialEnv(): Environment = {
    ...
    val uri = FijiURI.newBuilder(fijiURI).build()
    return preloadModules(new Environment(uri, Console.out, ShellMain.shellFijiSystem, input,
...
object ShellMain {
  ...
  /** Singleton FijiSystem instance used in interactive shell process. */
  val shellFijiSystem: AbstractFijiSystem = new FijiSystem
  ...
  /**
   * Main entry point for running the Fiji schema shell.
   *
