...
abstract class SchemaSpec {
   ...
   * @param env the operating Fiji shell environment.
  ...
  protected def addAvroToCellSchema(
    ...
    val uidForSchemaClass: Long = env.fijiSystem.getOrCreateSchemaId(env.instanceURI, schema)
   ...
   *     from.
   * @param the operating Fiji shell environment.
   * @return the modified CellSchema.
   */
  protected def dropAvroFromCellSchema(avroSchema: Schema, cellSchema: CellSchema,
    ...
    val maybeUidForSchemaClass: Option[Long] =
        env.fijiSystem.getSchemaId(env.instanceURI, avroSchema)
