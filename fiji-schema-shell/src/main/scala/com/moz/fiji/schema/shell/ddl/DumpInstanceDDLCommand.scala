...
final class DumpInstanceDDLCommand(val env: Environment, val maybeFile: Option[String])
  ...
  def dumpAllLayouts(): Unit = {
    val namesAndDescs = env.fijiSystem.getTableNamesDescriptions(getFijiURI())
    namesAndDescs.foreach { case (name, _) =>
      dumpLayout(env.fijiSystem.getTableLayout(getFijiURI(), name).getOrElse(
          // Shouldn't get this, since the FijiSystem just returned this name to us..
