...
final class InputProcessor(val throwOnErr: Boolean = false) {
  ...
  private def printHelp(env: Environment): Unit = {
    val helpLines = """
        ...
        |inspect, and delete Fiji table layouts.
        |
        |Commands in this terminal operate on the metadata maintained by FijiSchema;
        ...
        |available online at:
        |""".stripMargin +
        "  http://docs.fiji.org/userguides/schema/" +
        com.moz.fiji.schema.util.VersionInfo.getSoftwareVersion() + "/schema-shell-ddl-ref/\n" +
        """
      ...
      case Some(inputLine) => {
        ...
        if (Set("exit", "quit", "exit;", "quit;").contains(inputSoFar)) {
          ...
          try {
            ...
            case e: FijiInvalidNameException =>
