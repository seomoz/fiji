...
import com.moz.fiji.schema.shell.ddl._
...
/**
 * Parsers for elements of grammar common to many fiji-schema DDL commands. These
...
trait DDLParserHelpers extends JavaTokenParsers {
   ...
   *     'single quoted', and must match the Fiji name restrictions.
   **/
  def validatedNameFromOptionallyQuotedString: Parser[String] = (
    optionallyQuotedString ^^ (name => { FijiNameValidator.validateLayoutName(name); name })
  )

  /**
   * @return a matcher for a legal Fiji instance name.
   */
  def instanceName: Parser[String] = (
    optionallyQuotedString ^^ (name => { FijiNameValidator.validateFijiName(name); name })
  )

  /** @return a matcher for a legal Fiji table name. */
  def tableName: Parser[String] = validatedNameFromOptionallyQuotedString

  /** @return a matcher for a legal Fiji row key component name. */
  def rowKeyElemName: Parser[String] = validatedNameFromOptionallyQuotedString

  /** @return a matcher for a legal Fiji locality group name. */
  def localityGroupName: Parser[String] = validatedNameFromOptionallyQuotedString

  /** @return a matcher for a legal Fiji column family name. */
  def familyName: Parser[String] = validatedNameFromOptionallyQuotedString

  /** @return a matcher for a legal Fiji column qualifier name. */
