...
import com.moz.fiji.schema.shell.ddl.CompressionTypeToken._
...
/** Schemas for columns in a table are not validated, per legacy (FijiSchema 1.0) compatibility. */
...
/**
 * Parser for a fiji-schema DDL command.
 */
@ApiAudience.Private
final class DDLParser(val env: Environment)
  ...
  /**
   * List available Fiji instances. Recognizes a SHOW INSTANCES statement.
   */
