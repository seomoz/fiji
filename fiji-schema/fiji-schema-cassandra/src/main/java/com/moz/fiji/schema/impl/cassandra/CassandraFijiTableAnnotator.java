...
/** Cassandra implementation of {@link com.moz.fiji.schema.FijiTableAnnotator}. */
@ApiAudience.Private
public final class CassandraFijiTableAnnotator implements FijiTableAnnotator {

  static final String METATABLE_KEY_PREFIX = "fiji.schema.table_annotator.";
  ...
  /**
   * Validate that a given key is a valid FijiTableAnnotator annotation key.
   *
  ...
  /**
   * Check if a metaTableKey is a FijiTableAnnotator meta table key for a column annotation.
   *
   * @param metaTableKey the meta table key to check.
   * @return whether the given meta table key is a FijiTableAnnotator meta table key for a column
   *     annotation.
   */
  ...
  /**
   * Check if a metaTableKey is a FijiTableAnnotator meta table key for a table annotation.
   *
   * @param metaTableKey the meta table key to check.
   * @return whether the given meta table key is a FijiTableAnnotator meta table key for a table
   *     annotation.
   */
   ...
   * private for testing.
   *
   * @param table FijiTable in which the specified column lives. This will be used for
   ...
   * @throws com.moz.fiji.schema.NoSuchColumnException in case the column does not exist in the table.
   */
  static String getMetaTableKey(
   ...
   * FijiTableAnnotator meta table key. This method is package private for testing.
   *
   ...
   * FijiTableAnnotator meta table key. This method is package private for testing.
   *
   * @param table FijiTable in which the column from the specified key lives. This will be used
   ...
   * @throws com.moz.fiji.schema.NoSuchColumnException in case the column does not exist in the table.
   */
  static FijiColumnName columnFromMetaTableKey(
      final CassandraFijiTable table,
      final String metaTableKey
  ) throws NoSuchColumnException {
  ...
  /**
   * Initialize a new CassandraFijiTableAnnotator for the given table.
   *
   * @param table the table served by this ColumnAnnotator.
   */
  public CassandraFijiTableAnnotator(
      final CassandraFijiTable table
  ) {
    mTable = (CassandraFijiTable) table.retain();
  ...
  public void close() throws IOException {
    requireAndSetState(State.OPEN, State.CLOSED, "close a CassandraFijiTableAnnotator");
    mTable.release();
  }
  ...
  private void setKV(
    ...
    mTable.getFiji().getMetaTable().putValue(mTable.getName(), metaTableKey, Bytes.toBytes(value));
  }
  ...
  private void removeKV(
    ...
    mTable.getFiji().getMetaTable().removeValues(mTable.getName(), metaTableKey);
  }
  ...
  private String getKV(
    ...
    try {
      return Bytes.toString(
          mTable.getFiji().getMetaTable().getValue(mTable.getName(), metaTableKey));
  ...
  private Set<String> keySet() throws IOException {
    return mTable.getFiji().getMetaTable().keySet(mTable.getName());
  }

  /**
   * Get the FijiColumnName from the given meta table key. If the key component cannot be translated
   * to a FijiColumnName, the meta table key will be removed and this method will return null.
   *
   * @param metaTableKey key from which to exact the column name.
   * @return FijiColumnName stored in the given meta table key or null if the column name cannot be
   *     translated.
   * @throws java.io.IOException in case of an error removing a defunct key.
   */
  private FijiColumnName columnFromMetaTableKey(
      final String metaTableKey
  ) throws IOException {
    try {
  ...
  /** {@inheritDoc} */
  @Override
  public Set<FijiColumnName> removeColumnAnnotationsInFamily(
      final String family,
      final String exactKey
  ) throws IOException {
    requireState(State.OPEN, "remove annotation");
    final Set<FijiColumnName> removedColumns = Sets.newHashSet();
    for (String metaTableKey : keySet()) {
      if (isKCAColumnMetaTableKey(metaTableKey)
          && Objects.equal(exactKey, keyFromMetaTableKey(metaTableKey))) {
        final FijiColumnName column = columnFromMetaTableKey(metaTableKey);
  ...
  /** {@inheritDoc} */
  @Override
  public Map<FijiColumnName, Set<String>> removeColumnAnnotationsInFamilyStartingWith(
      final String family,
      final String prefix
  ) throws IOException {
    requireState(State.OPEN, "remove annotation");
    final Map<FijiColumnName, Set<String>> removedAnnotationKeys = Maps.newHashMap();
    for (String metaTableKey : keySet()) {
      if (isKCAColumnMetaTableKey(metaTableKey)) {
        final FijiColumnName column = columnFromMetaTableKey(metaTableKey);
        if (null != column && Objects.equal(family, column.getFamily())) {
  ...
  /** {@inheritDoc} */
  @Override
  public Map<FijiColumnName, Set<String>> removeColumnAnnotationsInFamilyContaining(
      final String family,
      final String substring
  ) throws IOException {
    requireState(State.OPEN, "remove annotation");
  ...
  /** {@inheritDoc} */
  @Override
  public Map<FijiColumnName, Set<String>> removeColumnAnnotationsInFamilyMatching(
      final String family,
      final String pattern
  ) throws IOException {
    requireState(State.OPEN, "remove annotation");
  ...
  /** {@inheritDoc} */
  @Override
  public Map<FijiColumnName, Set<String>> removeAllColumnAnnotationsInFamily(
      final String family
  ) throws IOException {
    requireState(State.OPEN, "remove annotation");
    final Map<FijiColumnName, Set<String>> removedAnnotationKeys = Maps.newHashMap();
  ...
  /** {@inheritDoc} */
  @Override
  public Map<FijiColumnName, Set<String>> removeAllColumnAnnotations() throws IOException {
    requireState(State.OPEN, "remove annotation");
    final Map<FijiColumnName, Set<String>> removedAnnotationKeys = Maps.newHashMap();
    for (String metaTableKey : keySet()) {
      if (isKCAColumnMetaTableKey(metaTableKey)) {
  ...
  /** {@inheritDoc} */
  @Override
  public Map<FijiColumnName, String> getColumnAnnotationsInFamily(
      final String family,
      final String exactKey
  ) throws IOException {
    requireState(State.OPEN, "get annotation");
  ...
  /** {@inheritDoc} */
  @Override
  public Map<FijiColumnName, Map<String, String>> getColumnAnnotationsInFamilyStartingWith(
      final String family,
      final String prefix
  ) throws IOException {
    requireState(State.OPEN, "get annotation");
  ...
  /** {@inheritDoc} */
  @Override
  public Map<FijiColumnName, Map<String, String>> getColumnAnnotationsInFamilyContaining(
      final String family,
      final String substring
  ) throws IOException {
    requireState(State.OPEN, "get annotation");
  ...
  /** {@inheritDoc} */
  @Override
  public Map<FijiColumnName, Map<String, String>> getColumnAnnotationsInFamilyMatching(
      final String family,
      final String pattern
  ) throws IOException {
    requireState(State.OPEN, "get annotation");
  ...
  /** {@inheritDoc} */
  @Override
  public Map<FijiColumnName, Map<String, String>> getAllColumnAnnotationsInFamily(
      final String family
  ) throws IOException {
    requireState(State.OPEN, "get annotation");
    final Map<FijiColumnName, Map<String, String>> collectedAnnotations = Maps.newHashMap();
  ...
  /** {@inheritDoc} */
  @Override
  public Map<FijiColumnName, Map<String, String>> getAllColumnAnnotations() throws IOException {
    requireState(State.OPEN, "get annotation");
    final Map<FijiColumnName, Map<String, String>> collectedAnnotations = Maps.newHashMap();
    for (String metaTableKey : keySet()) {
      if (isKCAColumnMetaTableKey(metaTableKey)) {
