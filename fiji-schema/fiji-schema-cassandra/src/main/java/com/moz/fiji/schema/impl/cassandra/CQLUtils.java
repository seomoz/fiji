 ...
 *
 * <p>
 *   Fiji RowKeyFormat2 defines 2 valid entity ID formats: formatted and raw.
 * </p>
 *
 * <ul>
 ...
 *
 *
 * <h2>Notes on Fiji Cassandra Tables</h2>
 *
 * <p>
 *   A single Fiji table is stored in Cassandra as multiple tables. There will be a Cassandra table
 *   per Fiji locality group.
 * </p>
 */
public final class CQLUtils {
   ...
   */
  public static List<String> getPartitionKeyColumns(FijiTableLayout layout) {
    RowKeyFormat2 keyFormat = (RowKeyFormat2) layout.getDesc().getKeysFormat();
    switch (keyFormat.getEncoding()) {
   ...
   */
  private static List<String> getEntityIdClusterColumns(FijiTableLayout layout) {
    RowKeyFormat2 keyFormat = (RowKeyFormat2) layout.getDesc().getKeysFormat();
    switch (keyFormat.getEncoding()) {
      case RAW: {
   ...
   * @return the primary key columns for the layout.
   */
  public static List<String> getLocalityGroupClusterColumns(FijiTableLayout layout) {
  ...
  /**
   * Return the CQL token column for a Fiji table layout.
   *
   * @param layout to create CQL token column for.
   * @return the CQL token column for the layout.
   */
  public static String getTokenColumn(FijiTableLayout layout) {
   ...
   * with CQL reserved words or with other column names in Fiji Cassandra tables.
   *
