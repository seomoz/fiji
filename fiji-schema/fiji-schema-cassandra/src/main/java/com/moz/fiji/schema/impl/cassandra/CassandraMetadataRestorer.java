 ...
 * com.moz.fiji.schema.impl.cassandra.CassandraMetadataRestorer#exportMetadata(String,
 * com.moz.fiji.schema.Fiji)} a filename and a Fiji instance. Backups are stored as {@link
 * com.moz.fiji.schema.avro.MetadataBackup} records. To restore a Fiji instance from a MetadataBackup
...
public class CassandraMetadataRestorer {
   ...
   * @throws java.io.IOException if there is an error communicating with HBase.
   */
  public void restoreTables(MetadataBackup backup, CassandraFiji fiji) throws IOException {
    final CassandraAdmin admin = fiji.getCassandraAdmin();
    final FijiMetaTable metaTable = fiji.getMetaTable();
    CassandraMetaTable.uninstall(admin, fiji.getURI());
    CassandraMetaTable.install(admin, fiji.getURI());
