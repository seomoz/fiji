...
/**
 * <p>The Fiji system table that is stored in Cassandra.</p>
 *
...
public final class CassandraSystemTable implements FijiSystemTable {
  ...
  /** The Cassandra row key that stores the installed Fiji data format version. */
  public static final String KEY_DATA_VERSION = "data-version";

  /** The Cassandra row key that stores the Fiji security version. */
  ...
  public static final String DEFAULTS_PROPERTIES_FILE =
      "com/moz/fiji/schema/system-default.properties";

  /** URI of the Fiji instance this system table belongs to. */
  private final FijiURI mInstanceURI;

  /** The Cassandra table that stores the Fiji instance properties. */
   ...
   * Fiji instance properties.
   *
   * @param instanceURI URI of the Fiji instance this table belongs to.
   * @param admin the Cassandra connection.
   */
  public CassandraSystemTable(FijiURI instanceURI, CassandraAdmin admin) {
    ...
    if (!mAdmin.tableExists(mTable)) {
      throw new FijiNotInstalledException("System table not installed.", mInstanceURI);
    }
  ...
  public synchronized void setSecurityVersion(ProtocolVersion version) throws IOException {
    ...
    Fiji.Factory.open(mInstanceURI).getSecurityManager().checkCurrentGrantAccess();
  ...
  public synchronized void close() throws IOException {
    ...
    Preconditions.checkState(oldState == State.OPEN,
        "Cannot close FijiSystemTable instance in state %s.", oldState);
  }
  ...
  /**
   * Installs a Fiji system table into a running HBase instance.
   *
  ...
  public static void install(CassandraAdmin admin, FijiURI fijiURI) throws IOException {
    install(admin, fijiURI, ImmutableMap.<String, String>of());
  }

  /**
   * Installs a Fiji system table into a running C* instance.
   *
   ...
   * @throws java.io.IOException If there is an error.
   */
  public static void install(CassandraAdmin admin, FijiURI fijiURI, Map<String, String> properties)
      throws IOException {
    // Install the table.  Sadly, we have to just use blobs and byte arrays here, so that we are
    // compliant with everything else in Fiji.  :(
    final CassandraTableName systemTableName = CassandraTableName.getSystemTableName(fijiURI);
    ...
    final CassandraSystemTable systemTable = new CassandraSystemTable(fijiURI, admin);
    try {
   ...
   * @param fijiURI The URI for the fiji instance to remove.
   * @throws java.io.IOException If there is an error.
   */
  public static void uninstall(
    ...
    final CassandraTableName tableName = CassandraTableName.getSystemTableName(fijiURI);
