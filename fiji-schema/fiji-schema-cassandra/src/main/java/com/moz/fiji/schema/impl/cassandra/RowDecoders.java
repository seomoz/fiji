...
public final class RowDecoders {
   ...
   * @param column The Fiji column name of the family.
   ...
   * @param layout The layout of the Fiji table.
   ...
   */
  public static <T> Function<ResultSet, Iterator<FijiCell<T>>> getColumnFamilyDecoderFunction(
    ...
    return new Function<ResultSet, Iterator<FijiCell<T>>>() {
      /** {@inheritDoc} */
      @Override
      public Iterator<FijiCell<T>> apply(final ResultSet resultSet) {
        ...
        try {
          if (layout.getFamilyMap().get(column.getFamily()).isMapType()) {
            // Map-type family
            final Function<Row, FijiCell<T>> decoder =
          ...
          throw new IllegalStateException(
              String.format("Column %s does not exist in Fiji table %s.",
   ...
   *
   * @param column The Fiji column of the Row.
   * @param decoderProvider A cell decoder provider for the table.
   * @param <T> The value type in the column.
   * @return A decoded cell.
   */
  public static <T> Function<ResultSet, Iterator<FijiCell<T>>> getQualifiedColumnDecoderFunction(
      final FijiColumnName column,
      final CellDecoderProvider decoderProvider
  ) {
   ...
   *
   * @param table The Fiji table the row keys belong to.
   ...
   */
  @NotThreadSafe
  private static final class MapFamilyDecoder<T> implements Function<Row, FijiCell<T>> {
     ...
     * @param familyColumn The Fiji column of the Row.
    ...
    public FijiCell<T> apply(final Row row) {
      ...
      try {
        ...
        return FijiCell.create(mLastColumn, version, decodedCell);
      } catch (IOException e) {
   ...
   */
  @NotThreadSafe
  private static final class GroupFamilyDecoder<T> implements Function<Row, FijiCell<T>> {
    private final CassandraTableName mTableName;
     ...
     * Fiji family, so we should not decode it. Currently the CassandraColumnNameTranslator does not
     * support this.
     *
   ...
   */
  @Immutable
  private static final class QualifiedColumnDecoder<T> implements Function<Row, FijiCell<T>> {
    private final FijiCellDecoder<T> mCellDecoder;
  ...
  /**
   * A 2-tuple combining a Cassandra token and Fiji row key components.
   */
  @Immutable
  public static class TokenRowKeyComponents {
    private final long mToken;
    private final FijiRowKeyComponents mComponents;
     ...
     */
    public TokenRowKeyComponents(final long token, final FijiRowKeyComponents components) {
      mToken = token;
     ...
     */
    public FijiRowKeyComponents getComponents() {
      return mComponents;
    }
  ...
  private static final class RawRowKeyDecoder implements Function<Row, TokenRowKeyComponents> {
     ...
     */
    private RawRowKeyDecoder(final FijiTableLayout layout) {
    ...
    public TokenRowKeyComponents apply(final Row row) {
      ...
      return new TokenRowKeyComponents(token, FijiRowKeyComponents.fromComponents(components));
    }
  }
  ...
  private static final class FormattedRowKeyDecoder
     ...
     * @param layout The table layout.
     */
    private FormattedRowKeyDecoder(final FijiTableLayout layout) {
      mTokenColumn = CQLUtils.getTokenColumn(layout);
    ...
    public TokenRowKeyComponents apply(final Row row) {
      ...
      return new TokenRowKeyComponents(
          ...
          FijiRowKeyComponents.fromComponents(components));
    }
  }
  ...
  private static class RowKeyComponentsToEntityId
     ...
     */
    public RowKeyComponentsToEntityId(final FijiTable table) {
      mTable = table;
    }
  ...
  /**
   * A predicate to filter excess Fiji Cells of a column from a Cassandra result set.
   */
  @NotThreadSafe
  ...
  /**
   * A predicate to filter Fiji cells below a minimum timestamp (inclusive).
   */
  @Immutable
  ...
  /**
   * A predicate to filter Fiji cells above a maximum timestamp (exclusive).
   */
  @Immutable
