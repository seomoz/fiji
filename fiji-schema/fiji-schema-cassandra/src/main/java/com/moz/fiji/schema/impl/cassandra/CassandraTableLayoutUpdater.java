...
import com.moz.fiji.schema.RuntimeInterruptedException;
...
import com.moz.fiji.schema.layout.impl.TableLayoutUpdateValidator;
import com.moz.fiji.schema.util.Lock;
import com.moz.fiji.schema.util.Time;
import com.moz.fiji.schema.zookeeper.TableLayoutTracker;
import com.moz.fiji.schema.zookeeper.TableLayoutUpdateHandler;
import com.moz.fiji.schema.zookeeper.UsersTracker;
import com.moz.fiji.schema.zookeeper.UsersUpdateHandler;
import com.moz.fiji.schema.zookeeper.ZooKeeperUtils;

/**
 * Updates the layout of a Cassandra Fiji table.
 *
 * <p>
...
public class CassandraTableLayoutUpdater {
  ...
  /**  */
  private final Function<FijiTableLayout, TableLayoutDesc> mLayoutUpdate;

  /** New table layout, set after the layout update completed. */
  private FijiTableLayout mNewLayout = null;
   ...
   *
   * @param fiji Opened Fiji instance the table belongs to.
  ...
  public CassandraTableLayoutUpdater(
      ...
      final Function<FijiTableLayout, TableLayoutDesc> layoutUpdate)
      throws IOException, KeeperException {
    mFiji = fiji;
    mFiji.retain();
    mTableURI = tableURI;
    mZKClient = mFiji.getZKClient();
  ...
  public CassandraTableLayoutUpdater(
      ...
      throws IOException, KeeperException {
    this(fiji, tableURI, new Function<FijiTableLayout, TableLayoutDesc>() {
      /** {@inheritDoc} */
      @Override
      public TableLayoutDesc apply(FijiTableLayout input) {
        return layoutUpdate;
      }
    });
  }
  ...
  public void update() throws IOException, KeeperException {
    ...
    try {
      ...
      final FijiTableLayout currentLayout = layoutMap.lastEntry().getValue();
      ...
      final TableLayoutUpdateValidator validator = new TableLayoutUpdateValidator(mFiji);
      final FijiTableLayout updatedLayout =
          FijiTableLayout.createUpdatedLayout(update , currentLayout);
      ...
      try {
        ...
        try {
          ...
          if (!Objects.equal(currentLayoutId, currentLayout.getDesc().getLayoutId())) {
            throw new InternalFijiError(String.format(
  ...
  private void writeMetaTable(TableLayoutDesc update) throws IOException {
    ...
    mNewLayout = mFiji.getMetaTable().updateTableLayout(table, update);
  }
   ...
   */
  public FijiTableLayout getNewLayout() {
    return mNewLayout;
  }

  /**
   * Validates that the provided Fiji table layout is valid for Fiji Cassandra.
   *
   * Currently Fiji Cassandra does not support the following:
   *
   * <ul>
        ...
        if (family.getMapSchema() != null) {
          ...
          if (family.getMapSchema().getType() == SchemaType.COUNTER) {
            throw new IllegalArgumentException(String.format(
                "Fiji Cassandra does not support counter map type families. Family: %s, table: %s.",
          ...
          for (final ColumnDesc column : family.getColumns()) {
            if (column.getColumnSchema().getType() == SchemaType.COUNTER) {
              throw new IllegalArgumentException(String.format(
                  "Fiji Cassandra does not support counter columns."
