...
import com.moz.fiji.schema.layout.impl.cassandra.CassandraTableLayoutDatabase;

/**
 * An implementation of the FijiMetaTable that uses the 'fiji-meta' C* table as the backing
 * store.
 */
@ApiAudience.Private
public final class CassandraMetaTable implements FijiMetaTable {
  ...
  /** The Cassandra table that stores layout-specific Fiji metadata. */
  private final CassandraTableName mLayoutTable;

  /** The Cassandra table that stores user-defined Fiji metadata. */
  ...
  // TODO: Make FijiTableLayoutDatabase thread-safe, so we can call CassandraMetaTable thread-safe.

  /**
   * Create a connection to a Fiji meta table backed by a C* table.
   *
   * <p>This class takes ownership of the C* table. It will be closed when this instance is
   * closed.</p>
   *
   * @param instanceURI URI of the Fiji instance this meta-table belongs to.
  ...
  public CassandraMetaTable(
    ...
    if (!admin.tableExists(mLayoutTable)) {
      throw new FijiNotInstalledException("Meta layout table not installed.", instanceURI);
    }
    if (!admin.tableExists(mKeyValueTable)) {
      throw new FijiNotInstalledException("Meta Key Value table not installed.", instanceURI);
    }
   ...
   * @throws java.io.IOException If there is an error.
   */
  public static void uninstall(CassandraAdmin admin, FijiURI uri) throws IOException {
