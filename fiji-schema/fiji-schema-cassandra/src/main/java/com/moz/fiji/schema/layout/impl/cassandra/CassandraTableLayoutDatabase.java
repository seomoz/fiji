...
import com.moz.fiji.schema.impl.AvroCellEncoder;
import com.moz.fiji.schema.impl.cassandra.CassandraAdmin;
...
import com.moz.fiji.schema.layout.TableLayoutBuilder.LayoutOptions;
import com.moz.fiji.schema.layout.TableLayoutBuilder.LayoutOptions.SchemaFormat;

/**
 * <p>Manages Fiji table layouts using a Cassandra table as a backing store.</p>
 *
 * <p>
 ...
 */
@ApiAudience.Private
public final class CassandraTableLayoutDatabase implements FijiTableLayoutDatabase {
  ...
  /** URI of the Fiji instance this layout database is for. */
  ...
  /** Fiji cell encoder. */
  private final FijiCellEncoder mCellEncoder;

  /** Decoder for concrete layout cells. */
  private final FijiCellDecoder<TableLayoutDesc> mCellDecoder;
   ...
   * @param uri The FijiURI of the instance for this table.
   */
  public static void install(CassandraAdmin admin, FijiURI uri) {
   ...
   *
   * @param fijiURI URI of the Fiji instance this layout database belongs to.
  ...
  public CassandraTableLayoutDatabase(
      ...
      throws IOException {
    mFijiURI = fijiURI;
    mMetaTableName = CassandraTableName.getMetaLayoutTableName(fijiURI);
    mAdmin = admin;
  ...
  /** {@inheritDoc} */
  @Override
  public FijiTableLayout updateTableLayout(String tableName, TableLayoutDesc layoutUpdate)
      throws IOException {
    ...
    final List<FijiTableLayout> layouts =
    ...
    final FijiTableLayout tableLayout = FijiTableLayout.createUpdatedLayout(update, currentLayout);
    ...
    for (FijiTableLayout layout : layouts) {
  ...
  /** {@inheritDoc} */
  @Override
  public FijiTableLayout getTableLayout(String table) throws IOException {
    final List<FijiTableLayout> layouts = getTableLayoutVersions(table, 1);
    if (layouts.isEmpty()) {
      throw new FijiTableNotFoundException(
          FijiURI.newBuilder(mFijiURI).withTableName(table).build());
    }
  ...
  /** {@inheritDoc} */
  @Override
  public List<FijiTableLayout> getTableLayoutVersions(String table, int numVersions)
      throws IOException {
    Preconditions.checkArgument(numVersions >= 1,  "numVersions must be positive");
    ...
    final List<FijiTableLayout> layouts = Lists.newArrayList();
    for (Row row: rows) {
      ByteBuffer blob = row.getBytes(QUALIFIER_LAYOUT);
      byte[] bytes = ByteUtils.toBytes(blob);
      layouts.add(FijiTableLayout.newLayout(decodeTableLayoutDesc(bytes)));
    }
    return layouts;
  }
  ...
  /** {@inheritDoc} */
  @Override
  public NavigableMap<Long, FijiTableLayout> getTimedTableLayoutVersions(
      String table,
      int numVersions
  ) throws IOException {
    Preconditions.checkArgument(numVersions >= 1, "numVersions must be positive");
    ...
    final NavigableMap<Long, FijiTableLayout> timedValues = Maps.newTreeMap();
    for (Row row: rows) {
      ByteBuffer blob = row.getBytes(QUALIFIER_LAYOUT);
      byte[] bytes = ByteUtils.toBytes(blob);
      FijiTableLayout layout = FijiTableLayout.newLayout(decodeTableLayoutDesc(bytes));
