...
/**
 * A cache for prepared CQL statements scoped to a single Fiji row key format.
 *
 * Responsibilities:
 *
 * <ul>
 ...
 *   instance of this class can be shared among all open Fiji tables which share a row key format.
 *   Fiji tables will often have a unique row key format in an instance, but this is not a
...
public class CQLStatementCache {
  ...
  /**
   * Prefix added to Fiji entity ID component names to make the Cassandra column name. Adding a
  ...
  /** Cassandra primary key columns belonging to the Fiji Entity ID. */
  private final List<String> mEntityIDColumns;

  /** Cassandra partition key columns. */
  private final List<String> mPartitionKeyColumns;

  /** Cassandra clustering columns belonging to the Fiji Entity ID. */
  ...
  /**
   * Create a new CQL statement cache for a Fiji table.
   *
  ...
  public CQLStatementCache(final Session session, final RowKeyFormat2 rowKeyFormat) {
    ...
    // Cassandra Fiji only supports RowKeyFormat2
  ...
  /**
   * Create a statement for retrieving a column in single row of a Cassandra Fiji table.
   *
   * @param tableName The Cassandra locality group table.
   * @param entityId The Fiji entity ID.
   * @param column The translated Fiji column name.
  ...
  /**
   * Create a CQL statement that executes a Fiji put.
   *
  ...
  /**
   * Create a CQL statement to delete a Fiji column from a row.
   *
   * @param table The name of the Cassandra table.
   * @param entityID The entity ID of the row to delete.
   * @param column The column to delete.
  ...
  /**
   * Create a CQL statement for deleting from a locality group in a row of a Cassandra Fiji table.
   *
   * @param table The name of the Cassandra table.
   * @param entityID The entity ID of the row to delete.
   * @param column The column to delete.
