...
class HFileTool extends BaseTool {
  ...
  @Flag(name="table",
      usage="""URI of a Fiji table to read from/write to.
        Only one of --fiji and --htable may be specified at a time.""")
  var tableFlag: String = null

  @Flag(name="hbase",
      usage="""URI of a Fiji instance to read from/write to.
  ...
  override def run(unparsed: JList[String]): Int = {
    ...
    if (tableFlag != null) {
      val tableURI = FijiURI.newBuilder(tableFlag).build()
      val fiji = Fiji.Factory.open(tableURI)
      try {
        val table = fiji.openTable(tableURI.getTable)
        try {
          val htable = table.asInstanceOf[HBaseFijiTable].openHTableConnection()
          try {
      ...
      val hbaseURI = FijiURI.newBuilder(hbaseFlag).build()
