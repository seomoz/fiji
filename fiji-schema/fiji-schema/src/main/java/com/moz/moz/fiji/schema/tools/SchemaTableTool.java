...
import com.moz.fiji.schema.FijiSchemaTable.SchemaEntry;
 ...
 * <pre>
 *   fiji schema-table fiji://hbase-address/instance-name/ \
...
public final class SchemaTableTool extends BaseTool {
  ...
  @Flag(name="list", usage="Set to true to list all the registered schemas in the fiji instance.")
  ...
  /** URI of the Fiji instance housing the target schema table. */
  private FijiURI mURI = null;

  /** Fiji instance housing the target schema table. */
  ...
  public String getDescription() {
    return "Inspect and modify a Fiji schema table.";
  }
  ...
  private int registerSchema() throws IOException {
    final FijiSchemaTable table = mFiji.getSchemaTable();
  ...
  private int list() throws IOException {
    final FijiSchemaTable schemaTable = mFiji.getSchemaTable();
    long id = 0;
  ...
  protected int run(List<String> nonFlagArgs) throws Exception {
    Preconditions.checkArgument(nonFlagArgs.size() != 0, "Specify the FijiURI of your target "
        + "instance with `fiji get fiji://hbase-cluster/instance`");
    mURI = FijiURI.newBuilder(Preconditions.checkNotNull(nonFlagArgs.get(0))).build();
    mFiji = Fiji.Factory.open(mURI, getConf());
    try {
      if (mRegisterFlag != null && !mRegisterFlag.isEmpty()) {
        ...
        throw new InternalFijiError("No operation specified.");
      }
    } finally {
      mFiji.release();
    }
