 ...
 * the <tt>bin/fiji</tt> script. These tools are instantiated and run by the
 * {@link FijiToolLauncher}.
 *
 * <p>To register a tool to use with <tt>bin/fiji</tt>, you must also put the complete
 * name of the implementing class in a resource in your jar file at:
 * <tt>META-INF/services/com.moz.fiji.schema.tools.FijiTool</tt>. You may publish multiple
...
public interface FijiTool extends Configurable {
  /**
   * @return the name of the tool. This name is used by users to run the tool.
   *     For example, to provide the <tt>fiji ls</tt> tool, this should return "ls".
   */
  String getName();
