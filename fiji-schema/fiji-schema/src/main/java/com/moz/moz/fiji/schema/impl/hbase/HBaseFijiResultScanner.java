 ...
 * {@inheritDoc}
 *
 * HBase implementation of {@code FijiResultScanner}.
 *
 * @param <T> type of {@code FijiCell} value returned by scanned {@code FijiResult}s.
 */
public class HBaseFijiResultScanner<T> implements FijiResultScanner<T> {
  private static final Logger LOG = LoggerFactory.getLogger(HBaseFijiResultScanner.class);
  ...
  private final FijiDataRequest mRequest;
  ...
  private void reopenScanner() {
    if (mReopenScannerOnTimeout) {
      ...
      throw new FijiIOException("HBase scanner timed out with automatic reopening disabled.");
    }
  }
  ...
  public FijiResult<T> next() {
    ...
    Preconditions.checkState(oldState == State.OPEN,
        "Cannot get element from FijiResultScanner in state %s.", oldState);
  ...
  public void close() throws IOException {
    ...
    Preconditions.checkState(oldState == State.OPEN,
        "Cannot close FijiResultScanner instance in state %s.", oldState);
    mResultScanner.close();
    mHTable.close();
  }
}
