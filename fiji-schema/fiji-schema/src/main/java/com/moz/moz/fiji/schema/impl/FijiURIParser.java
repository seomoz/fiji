...
import com.moz.fiji.delegation.NamedProvider;
import com.moz.fiji.delegation.NoSuchProviderException;
import com.moz.fiji.schema.FijiColumnName;
import com.moz.fiji.schema.FijiURI;
import com.moz.fiji.schema.FijiURI.FijiURIBuilder;
import com.moz.fiji.schema.FijiURIException;
import com.moz.fiji.schema.hbase.HBaseFijiURI;

/**
 * A Parser for {@link FijiURI}s in text form.  {@code FijiURIParser}s register themselves to
 * handle parsing of certain schemes, and the correct parser to use for a given string is decided
 * by its scheme.
 */
@ApiAudience.Private
public interface FijiURIParser extends NamedProvider {
  /**
   * Parse a URI and return a {@link FijiURIBuilder}.
   *
   * @param uri to parse.
   * @return a FijiURIBuilder parsed from the URI.
   */
  FijiURIBuilder parse(URI uri);

  /**
   * A factory for retrieving a {@link FijiURIParser} for a URI, and parsing it.
   */
  public static class Factory {
    /**
     * Parse a {@link URI} into a {@link FijiURIBuilder} based on the {@link URI}'s scheme.
     *
     * @param uri to parse.
     * @return a {@link FijiURIBuilder} for the uri.
     */
    public static FijiURIBuilder get(URI uri) {
      ...
      try {
        return Lookups.getNamed(FijiURIParser.class).lookup(scheme).parse(uri);
      } catch (NoSuchProviderException e) {
        throw new FijiURIException(
            uri.toString(),
            String.format("No parser available for Fiji scheme '%s'.", scheme));
      }
    }
  }
  ...
  /**
   * Class for parsing an authority portion in a FijiURI.
   *
   * An HBase FijiURI has only a single authority, whereas a Cassandra FijiURI has two, one for
   * ZooKeeper and another for Cassandra (the Cassandra FijiURI is not strictly a legal URI).
   *
   * The authority in a Cassandra FijiURI can also have a username and password.
   */
  public static final class AuthorityParser {
     ...
     * @throws FijiURIException If the authority is invalid.
     */
     ...
     *
     * We specify the authority explicitly because in a Cassandra FijiURI, the Cassandra hosts
    ...
    private static ImmutableList<String> parseHosts(URI uri, String segment) {
      final Matcher zkMatcher = FijiURI.RE_AUTHORITY_GROUP.matcher(segment);
  ...
  public static final class ZooKeeperAuthorityParser {
    ...
    static {
      ...
      ENV_ZOOKEEPER_CLIENT_PORT =
          conf.getInt(HConstants.ZOOKEEPER_CLIENT_PORT, FijiURI.DEFAULT_ZOOKEEPER_CLIENT_PORT);
    }
    ...
    private ZooKeeperAuthorityParser(URI uri) {
      ...
      if (null == authority) {
        throw new FijiURIException(uri.toString(), "ZooKeeper ensemble missing.");
      }

      if (authority.equals(FijiURI.ENV_URI_STRING)) {
        ...
        mZookeeperClientPort = port == null ? FijiURI.DEFAULT_ZOOKEEPER_CLIENT_PORT : port;
  ...
  /**
   * Parses {@code URI} path segments into Fiji componenets.
   */
  public static class PathParser {

    private final String mInstance;
    private final String mTable;
    private final List<FijiColumnName> mColumns;

    /**
     * Create a new path segment parser for a FijiURI.
     *
     * @param uri to parse.
     */
    ...
    /**
     * Create a new path segment parser for a FijiURI with the provided instanceIndex.  The Fiji
     * components begin at the {@code instanceIndex} offset of the path segments.
     *
     * @param uri to parse.
     * @param instanceIndex of Fiji component.
     */
    public PathParser(URI uri, int instanceIndex) {
      ...
      if (size - instanceIndex > 3) {
        throw new FijiURIException(uri.toString(), String.format("Too many path segments."));
      }
      final int tableIndex = instanceIndex + 1;
      final int columnsIndex = tableIndex + 1;

      if (size > instanceIndex  && !segments.get(instanceIndex).equals(FijiURI.UNSET_URI_STRING)) {
        mInstance = segments.get(instanceIndex);
      } else {
        mInstance = null;
      }

      if (size > tableIndex && !segments.get(tableIndex).equals(FijiURI.UNSET_URI_STRING)) {
        mTable = segments.get(tableIndex);
      } else {
        mTable = null;
      }

      if (size > columnsIndex && !segments.get(columnsIndex).equals(FijiURI.UNSET_URI_STRING)) {
        ImmutableList.Builder<FijiColumnName> columns = ImmutableList.builder();
        for (String column
            : Splitter.on(',').omitEmptyStrings().split(segments.get(columnsIndex))) {
          columns.add(FijiColumnName.create(column));
        }
     ...
     */
    public List<FijiColumnName> getColumns() {
      return mColumns;
    }
  }
}
