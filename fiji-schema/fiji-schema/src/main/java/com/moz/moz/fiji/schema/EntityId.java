...
/**
 * EntityId is used to identify a particular row under the key format of a Fiji table.
 *
 * There are two name-spaces for rows:
 * <ul>
 *   <li> Fiji rows are logically indexed by Fiji row keys (arbitrary byte arrays or a list of
 *        key components).
 *   <li> Under the hood, rows are indexed by HBase row keys (arbitrary byte arrays).
 * </ul>
 *
 * The translation between Fiji row keys and HBase row keys depends on the layout of the table
 * the row belongs to. The same Fiji row key may translate to two different EntityIds on two
 * different tables, based on their key format. For a representation of row keys which is
 * agnostic to row key formats, see {@link FijiRowKeyComponents}.
 *
 * There are multiple translation schemes:
 * <ul>
 *   <li> Raw: Fiji row keys and HBase row keys are identical (identity translation), specifically
 *   used when the row key is an array of bytes.
 *   <li> Hash: HBase row keys are MD5 hashes of the Fiji row key (non reversible transform).
 *   <li> Hash-prefix: HBase row keys are Fiji row keys prefixed by a hash of the Fiji row key.
...
public abstract class EntityId {
  ...
  /**
   * Translates this Fiji row key into an HBase row key.
   *
  ...
  /**
   * Get the individual components of the fiji Row Key representation. This excludes hash
