...
import com.moz.fiji.schema.layout.impl.TableLayoutMonitor.DefaultTableLayoutMonitor;
import com.moz.fiji.schema.layout.impl.TableLayoutMonitor.ReferencedTableLayoutMonitor;
import com.moz.fiji.schema.util.JvmId;
import com.moz.fiji.schema.util.ProtocolVersion;
import com.moz.fiji.schema.zookeeper.InstanceUserRegistration;

/**
 * A Fiji instance monitor. Registers a client as an instance user in ZooKeeper, and provides
...
public final class InstanceMonitor implements Closeable {
  ...
  /**
   * Create an instance monitor for a Fiji instance.
   *
  ...
  public TableLayoutMonitor getTableLayoutMonitor(String tableName) {
    ...
    LOG.debug("Retrieving TableLayoutMonitor for table {} with userID {}.",
        FijiURI.newBuilder(mInstanceURI).withTableName(tableName).build(), mUserID);
  ...
  private static String generateInstanceUserID() {
    return String.format("%s;HBaseFiji@%s", JvmId.get(), INSTANCE_COUNTER.getAndIncrement());
  }
  ...
  private final class TableLayoutMonitorFactoryFn implements Function<String, TableLayoutMonitor> {
    ...
    public TableLayoutMonitor apply(String tableName) {
      final FijiURI tableURI = FijiURI.newBuilder(mInstanceURI).withTableName(tableName).build();
      try {
