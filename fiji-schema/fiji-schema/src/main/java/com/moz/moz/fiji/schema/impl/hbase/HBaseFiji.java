...
import com.moz.fiji.schema.layout.impl.HTableSchemaTranslator;
 ...
 * Multiple instances of Fiji can be installed onto a single HBase cluster.
 ...
 */
@ApiAudience.Private
public final class HBaseFiji implements Fiji {
  private static final Logger LOG = LoggerFactory.getLogger(HBaseFiji.class);
  ...
  /** URI for this HBaseFiji instance. */
  ...
  private static enum State {
     ...
     * incremented.  Resources are successfully opened and this HBaseFiji's methods may be used.
     */
    OPEN,
    /**
  ...
  /** Retain counter. When decreased to 0, the HBase Fiji may be closed and disposed of. */
  ...
  /** The schema table for this fiji instance. */
  ...
  /** The meta table for this fiji instance. */
  ...
  /**
   * Creates a new <code>HBaseFiji</code> instance.
   *
  ...
  /** {@inheritDoc} */
  @Override
  public FijiSchemaTable getSchemaTable() throws IOException {
  ...
  /** {@inheritDoc} */
  @Override
  public FijiMetaTable getMetaTable() throws IOException {
    final State state = mState.get();
    Preconditions.checkState(state == State.OPEN,
  ...
  /**
   * Gets the current HBaseAdmin instance for this Fiji. This method will open a new
   * HBaseAdmin if one doesn't exist already.
   *
   * @throws IOException If there is an error opening the HBaseAdmin.
   * @return The current HBaseAdmin instance for this Fiji.
   */
  ...
  public synchronized FijiSecurityManager getSecurityManager() throws IOException {
    ...
    if (null == mSecurityManager) {
      if (isSecurityEnabled()) {
        mSecurityManager = FijiSecurityManager.Factory.create(mURI, getConf(), mHTableFactory);
      } else {
  ...
  /** {@inheritDoc} */
  @Override
  public HBaseFijiTable openTable(String tableName) throws IOException {
    final State state = mState.get();
    Preconditions.checkState(state == State.OPEN,
        "Cannot open table in Fiji instance %s in state %s.", this, state);

    if (!getTableNames().contains(tableName)) {
      throw new FijiTableNotFoundException(
          FijiURI.newBuilder(mURI).withTableName(tableName).build());
    }

    return new HBaseFijiTable(
        this,
        tableName,
        mConf,
        mHTableFactory,
  ...
  @Deprecated
  @Override
  public void createTable(String tableName, FijiTableLayout tableLayout, int numRegions)
      throws IOException {
    if (!tableName.equals(tableLayout.getName())) {
      throw new RuntimeException(String.format(
          "Table name from layout descriptor '%s' does match table name '%s'.",
  ...
  public void createTable(TableLayoutDesc tableLayout, int numRegions)
    ...
    if (numRegions > 1) {
      if (FijiTableLayout.getEncoding(tableLayout.getKeysFormat())
          == RowKeyEncoding.RAW) {
        throw new IllegalArgumentException(
            "May not use numRegions > 1 if row key hashing is disabled in the layout");
      }

      createTable(tableLayout, FijiRowKeySplitter.get().getSplitKeys(numRegions,
          FijiRowKeySplitter.getRowKeyResolution(tableLayout)));
    } else {
  ...
  @Deprecated
  @Override
  public void createTable(String tableName, FijiTableLayout tableLayout, byte[][] splitKeys)
      throws IOException {
    if (getMetaTable().tableExists(tableName)) {
      final FijiURI tableURI =
          FijiURI.newBuilder(mURI).withTableName(tableName).build();
      throw new FijiAlreadyExistsException(String.format(
          "Fiji table '%s' already exists.", tableURI), tableURI);
    }
  ...
  public FijiTableLayout modifyTableLayout(
    ...
    final FijiManagedHBaseTableName hbaseTableName =
        FijiManagedHBaseTableName.getFijiTableName(mURI.getInstance(), tableName);
    ...
    if (currentTableDescriptor == null) {
      if (dryRun) {
        ...
        throw new RuntimeException("Table " + hbaseTableName.getFijiTableName()
            + " does not exist");
      }
    }
  ...
  // CSOFF: MethodName
  /**
   * Attempt to delete a table from this System_1_0 Fiji instance.
   *
  ...
  private void deleteTableSystem_1_0(String tableName) throws IOException {
    // Delete from HBase.
    String hbaseTable =
        FijiManagedHBaseTableName.getFijiTableName(mURI.getInstance(), tableName).toString();
  ...
  /**
   * Attempt to delete a table from this System_2_0 Fiji instance.
   *
   * @param tableName to delete
   * @throws IOException on unrecoverable error.
   */
  private void deleteTableSystem_2_0(String tableName) throws IOException {
    ...
    try {
      ...
      if (!users.isEmpty()) {
        LOG.warn(
            "Uninstalling Fiji table '{}' with registered users."
  ...
  public String toString() {
    return Objects.toStringHelper(HBaseFiji.class)
  ...
  /**
   * Creates a Fiji table in an HBase instance, without checking for validation compatibility and
  ...
  private void createTableUnchecked(
    ...
    final FijiTableLayout fijiTableLayout = FijiTableLayout.newLayout(tableLayout);
    ...
    if (mSystemVersion.compareTo(Versions.SYSTEM_2_0) >= 0) {
      ...
      try {
        ZooKeeperUtils.setTableLayout(
            mZKClient,
            tableURI,
            fijiTableLayout.getDesc().getLayoutId());
      } catch (Exception e) {
    ...
    try {
      ...
      final HTableDescriptor desc =
          translator.toHTableDescriptor(tableURI.getInstance(), fijiTableLayout);
