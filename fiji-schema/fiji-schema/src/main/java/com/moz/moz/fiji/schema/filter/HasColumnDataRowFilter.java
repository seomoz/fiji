...
/**
 * A FijiRowFilter that excludes rows that have no data for some column <code>columnName</code>.
 */
@ApiAudience.Public
@ApiStability.Evolving
public final class HasColumnDataRowFilter extends FijiRowFilter {
  ...
  public HasColumnDataRowFilter(String columnName) {
    ...
    FijiColumnName fijiColName = FijiColumnName.create(columnName);
    if (!fijiColName.isFullyQualified()) {
      throw new IllegalArgumentException("Cannot use an unqualified column family.");
    }

    mFamily = fijiColName.getFamily();
    mQualifier = fijiColName.getQualifier();
  }
  ...
  public Filter toHBaseFilter(Context context) throws IOException {
    ...
    // not an empty byte array.  Since all fiji cells have data (at the very least, the
  ...
  public static final class HasColumnDataRowFilterDeserializer
      implements FijiRowFilterDeserializer {
    /** {@inheritDoc} */
    @Override
