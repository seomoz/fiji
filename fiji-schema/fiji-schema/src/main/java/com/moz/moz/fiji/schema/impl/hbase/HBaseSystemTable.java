...
/**
 * <p>The Fiji system table that is stored in HBase.</p>
 *
 ...
 */
@ApiAudience.Private
public final class HBaseSystemTable implements FijiSystemTable {
  ...
  /** The HBase row key that stores the installed Fiji data format version. */
  public static final String KEY_DATA_VERSION = "data-version";

  /** The HBase row key that stores the Fiji security version. */
  ...
  /** The HTable that stores the Fiji instance properties. */
  ...
  /**
   * Creates a new HTableInterface for the Fiji system table.
   *
   * @param fijiURI The FijiURI.
   * @param conf The Hadoop configuration.
   * @param factory HTableInterface factory.
   * @return a new HTableInterface for the Fiji system table.
   * @throws IOException on I/O error.
   * @throws FijiNotInstalledException if the Fiji instance does not exist.
   */
  public static HTableInterface newSystemTable(
    ...
    final String tableName =
        FijiManagedHBaseTableName.getSystemTableName(fijiURI.getInstance()).toString();
    try {
  ...
  /**
   * Connect to the HBase system table inside a Fiji instance.
   *
   * @param fijiURI The FijiURI.
   * @param conf the Hadoop configuration.
   * @param factory HTableInterface factory.
  ...
  public HBaseSystemTable(
      ...
      HTableInterfaceFactory factory)
      throws IOException {
    this(fijiURI, newSystemTable(fijiURI, conf, factory));
  }
   ...
   *
   * @param uri URI of the Fiji instance this table belongs to.
   * @param htable An HTable to wrap.
   */
  public HBaseSystemTable(FijiURI uri, HTableInterface htable) {
    mURI = uri;
    mTable = htable;
  ...
  public static void install(
      ...
      HTableInterfaceFactory factory)
      throws IOException {
    install(admin, fijiURI, conf, Collections.<String, String>emptyMap(), factory);
  }
  ...
  public static void install(
    ...
    HBaseSystemTable systemTable = new HBaseSystemTable(fijiURI, conf, factory);
    try {
   ...
   * @throws IOException If there is an error.
   */
  public static void uninstall(HBaseAdmin admin, FijiURI fijiURI)
      throws IOException {
    final String tableName =
        FijiManagedHBaseTableName.getSystemTableName(fijiURI.getInstance()).toString();
