...
/**
 * Main entry point to launch Fiji tools.
 *
 * <p>All tools launched through <tt>bin/fiji &lt;tool&gt;</tt> are detected and
 * instantiated through this module. Tools should implement the {@link FijiTool}
 * interface. In addition, each tool must advertise itself by adding a line to
 * a resource file contained in its jar at
 * <tt>META-INF/services/com.moz.fiji.schema.tools.FijiTool</tt>.
 * This path can be added to your build by providing it under <tt>src/main/resources</tt>.</p>
 */
@ApiAudience.Public
@ApiStability.Evolving
public final class FijiToolLauncher extends Configured {
  ...
  public int run(String[] args) throws Exception {
    if (args.length == 0) {
      System.err.println("Error: Must run 'fiji <toolName>'.");
    ...
    FijiTool tool = getToolForName(toolName);
   ...
   * @return the FijiTool that provides for that name, or null if none does.
   */
  FijiTool getToolForName(String toolName) {
    FijiTool tool = null;

    // Iterate over available tools, searching for the one with
    // the same name as the supplied tool name argument.
    for (FijiTool candidate : Lookups.get(FijiTool.class)) {
   ...
   * in a manner similar to Hadoop's ToolRunner.
   *
   * @param tool The FijiTool to run.
  ...
  public static void main(String[] args) throws Exception {
    System.exit(new FijiToolLauncher().run(args));
  }
}
