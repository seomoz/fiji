...
public final class RawEntityId extends EntityId {
  /**
   * Creates a RawEntityId from the specified Fiji row key.
   *
   * @param fijiRowKey Fiji row key.
   * @return a new RawEntityId with the specified Fiji row key.
   */
  static RawEntityId getEntityId(byte[] fijiRowKey) {
    return new RawEntityId(fijiRowKey);
  }
   ...
   *
   * @param rowKey Fiji/HBase row key (both row keys are identical).
   */
