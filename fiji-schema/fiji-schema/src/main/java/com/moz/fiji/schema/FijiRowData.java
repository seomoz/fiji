...
/**
 * FijiRowData provides a way for applications to access data read from a Fiji table.
 * FijiRowData objects contain a subset of the cells contained within a row in
 * a Fiji table. This subset is often determined by a {@link FijiDataRequest} object
 * that must be provided before a read operation.
 *
 * <p>
 *   FijiRowData objects should not be constructed directly, but rather should be
 *   obtained from a read operation. FijiRowData objects can be obtained by using a
 *   {@link FijiTableReader}, {@link FijiRowScanner}, or {@link FijiPager}.
 * </p>
 *
 * <h2>Reading cells</h2>
 * <p>
 *   Rows read from a Fiji table may contain multiple cells from multiple columns. To read the
 *   value from a particular column:
 * </p>
 * <pre>
 *   <code>
 *     final FijiRowData row = myTableReader.get(myEntityId, myDataRequest);
 *
 ...
 * <p>
 *   Since rows within a Fiji table may not contain all the columns defined within a layout
 *   we need a way to check this in code. FijiRowData contains several methods to help with
 *   this:
 * </p>
 * <pre>
 *   <code>
 ...
 * <p>
 *   Rows in Fiji tables may contain large amounts of data. In some cases, this is caused by
 ...
 *   To deal with large rows, you may use paging through a {@link FijiPager}.
 ...
 *   See the documentation for {@link FijiPager} for more information.
 * </p>
 */
@ApiAudience.Public
@ApiStability.Stable
@Inheritance.Sealed
public interface FijiRowData {
  /**
   ...
   * @throws IOException If there is an error.
   */
  <T> FijiCell<T> getCell(String family, String qualifier, long timestamp)
      throws IOException;
   ...
   * @throws IOException If there is an error.
   */
  <T> FijiCell<T> getMostRecentCell(String family, String qualifier)
      throws IOException;
   ...
   * @throws IOException If there is an error.
   */
  <T> NavigableMap<String, FijiCell<T>> getMostRecentCells(String family)
      throws IOException;
   ...
   * @throws IOException If there is an error.
   */
  <T> NavigableMap<String, NavigableMap<Long, FijiCell<T>>> getCells(String family)
      throws IOException;
   ...
   * @throws IOException If there is an error.
   */
  <T> NavigableMap<Long, FijiCell<T>> getCells(String family, String qualifier)
      throws IOException;

  /**
   * Gets a FijiPager for the specified column.
   *
   * <p>
   ...
   * </p>
   * <p> See {@link FijiPager} for more details on pagers. </p>
   * <p>
   ...
   * @throws FijiColumnPagingNotEnabledException If paging is not enabled for the
   *     specified column.
   *
   * @see FijiPager
   ...
   */
  FijiPager getPager(String family, String qualifier)
      throws FijiColumnPagingNotEnabledException;

  /**
   * Gets a FijiPager for the specified column family.
   *
   * <p>
   ...
   * @see MapFamilyVersionIterator
   */
  FijiPager getPager(String family)
      throws FijiColumnPagingNotEnabledException;
   ...
   * @throws IOException If there is an error.
   */
  <T> Iterator<FijiCell<T>> iterator(String family, String qualifier) throws IOException;
   ...
   * @throws IOException If there is an error.
   */
  <T> Iterator<FijiCell<T>> iterator(String family) throws IOException;
   ...
   * @throws IOException If there is an error.
   */
  <T> Iterable<FijiCell<T>> asIterable(String family, String qualifier) throws IOException;
   ...
   * @throws IOException If there is an error.
   */
  <T> Iterable<FijiCell<T>> asIterable(String family) throws IOException;
}
