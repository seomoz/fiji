...
public abstract class EntityIdFactory {
   ...
   * table layout.
   *
   * @param fijiTableLayout the layout of the fiji table.
   * @return a new entity ID factory.
   */
  public static EntityIdFactory getFactory(FijiTableLayout fijiTableLayout) {
    Object rowKeyFormat = fijiTableLayout.getDesc().getKeysFormat();
    if (rowKeyFormat instanceof RowKeyFormat) {
      ...
      throw new RuntimeException("Fiji Table has unknown RowKeyFormat"
  ...
  /**
   * Creates an entity ID from a FijiRowKeyComponents.
   *
   * @param fijiRowKeyComponents This should be constructed with one of the following,
   ...
   *            in their expected order in the key. </li>
   *     </ul>
   * @return a new EntityId for the specified FijiRowKeyComponents.
   */
  public EntityId getEntityId(FijiRowKeyComponents fijiRowKeyComponents) {
    return getEntityId(fijiRowKeyComponents.getComponents());
  }
