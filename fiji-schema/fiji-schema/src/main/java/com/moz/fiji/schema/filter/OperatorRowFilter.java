 ...
 */
@ApiAudience.Private
abstract class OperatorRowFilter extends FijiRowFilter {
  ...
  private final FijiRowFilter[] mFilters;
   ...
   */
  OperatorRowFilter(Operator operator, FijiRowFilter... filters) {
  ...
  public FijiDataRequest getDataRequest() {
    FijiDataRequest dataRequest = FijiDataRequest.builder().build();
   ...
   */
  protected static List<FijiRowFilter> parseFilterList(JsonNode root) {
    ...
    final List<FijiRowFilter> filters = Lists.newArrayList();
    for (JsonNode filterNode : filtersNode) {
      ...
      filters.add(FijiRowFilter.toFilter(filterNode));
    }
    return filters;
  }
}
