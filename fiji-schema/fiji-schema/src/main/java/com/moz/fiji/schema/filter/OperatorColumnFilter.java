 ...
 */
@ApiAudience.Private
class OperatorColumnFilter extends FijiColumnFilter {
  ...
  private final FijiColumnFilter[] mFilters;
   ...
   */
  OperatorColumnFilter(Operator operator, FijiColumnFilter[] filters) {
  ...
  public Filter toHBaseFilter(FijiColumnName fijiColumnName, Context context) throws IOException {
    ...
    for (FijiColumnFilter filter : mFilters) {
      if (filter != null) {
        hbaseFilters.add(filter.toHBaseFilter(fijiColumnName, context));
      }
    }
