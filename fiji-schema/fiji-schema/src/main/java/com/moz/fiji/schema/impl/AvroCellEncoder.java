...
import com.moz.fiji.schema.impl.hbase.HBaseTableLayoutUpdater;
import com.moz.fiji.schema.layout.AvroSchemaResolver;
import com.moz.fiji.schema.layout.CellSpec;
import com.moz.fiji.schema.layout.InvalidLayoutException;
import com.moz.fiji.schema.layout.FijiTableLayout;
import com.moz.fiji.schema.layout.SchemaTableAvroResolver;
import com.moz.fiji.schema.layout.TableLayoutBuilder;
import com.moz.fiji.schema.util.ByteStreamArray;
 ...
 *
 * <p>
 *   An Avro cell encoder is specific to one column in a FijiTable.
 ...
 */
@ApiAudience.Private
public final class AvroCellEncoder implements FijiCellEncoder {
  /** Name of the system property to control schema validation. */
  public static final String SCHEMA_VALIDATION_POLICY =
      "com.moz.fiji.schema.impl.AvroCellEncoder.SCHEMA_VALIDATION_POLICY";
  ...
  private static AvroValidationPolicy getAvroValidationPolicy(final CellSpec cellSpec) {
    ...
    if (validationPolicy != null) {
      try {
        ...
        throw new FijiEncodingException(
   ...
   * "com.moz.fiji.schema.impl.AvroCellEncoder.SCHEMA_VALIDATION_POLICY" system property.
   */
  ...
  public synchronized <T> byte[] encode(final T cellValue) throws IOException {
      ...
      case DEVELOPER: {
        if (!mRegisteredWriters.contains(writerSchema)) {
          ...
          if (mCellSpec.getColumnURI() == null) {
            throw new InternalFijiError("CellSpec has no column URI: " + mCellSpec);
          }
    ...
    try {
      ...
      throw new FijiEncodingException(cce);
    } catch (AvroRuntimeException ure) {
      throw new FijiEncodingException(ure);
    }
   ...
   * @throws FijiEncodingException if no Avro writer schema can be inferred from cellValue.
   */
  private <T> Schema getWriterSchema(final T cellValue) {
    if (cellValue == null) {
      ...
      if (schema == null) {
        throw new FijiEncodingException(String.format(
   ...
   * @throws IOException on I/O error.
   */
  private static void registerWriterSchema(final FijiURI columnURI, final Schema writerSchema)
      throws IOException {
    Preconditions.checkArgument(columnURI.getColumns().size() == 1,
        "Expecting exactly one column in URI, got: %s", columnURI);
    final FijiColumnName column = columnURI.getColumns().get(0);

    // TODO(???) the layout updater interface is currently HBase specific.
    //     We should make a backend agnostic API for layout updates.
    final HBaseFiji fiji = (HBaseFiji) Fiji.Factory.open(columnURI);
    try {
      final Function<FijiTableLayout, TableLayoutDesc> update =
          new Function<FijiTableLayout, TableLayoutDesc>() {
            /** {@inheritDoc} */
            @Override
            public TableLayoutDesc apply(final FijiTableLayout refLayout) {
              Preconditions.checkNotNull(refLayout);
              try {
                final TableLayoutDesc refDesc = refLayout.getDesc();
                return new TableLayoutBuilder(refDesc, fiji)
      ...
      try {
        final HBaseTableLayoutUpdater updater =
            new HBaseTableLayoutUpdater(fiji, columnURI, update);
        try {
          updater.update();
        } finally {
          updater.close();
