...
public final class TableLayoutBuilder {
  ...
  /** Function that resolves AvroSchema to Schema objects against the specified Fiji instance. */
   ...
   */
  public TableLayoutBuilder(FijiSchemaTable schemaTable) throws IOException {
   ...
   * @param fiji the instance in which to configure schemas
   * @throws IOException on I/O error.
   *     In particular, throws InvalidLayoutException if validation is not supported.
   */
  public TableLayoutBuilder(TableLayoutDesc tableLayoutDesc, Fiji fiji)
      throws IOException {
    ...
    mSchemaTable = fiji.getSchemaTable();
   ...
   */
  private FamilyDesc getFamilyDesc(final FijiColumnName family) throws NoSuchColumnException {
   ...
   */
  private CellSchema getColumnSchema(final FijiColumnName columnName)
   ...
   * @throws IOException if the cellSchema of the column can not be acquired.
   */
  public Collection<Schema> getRegisteredReaders(final FijiColumnName columnName)
      throws IOException {
   ...
   * @throws IOException if the cellSchema of the column can not be acquired.
   */
  public Collection<Schema> getRegisteredWriters(final FijiColumnName columnName)
      throws IOException {
   ...
   * @throws IOException if the cellSchema of the column can not be acquired.
   */
  public Collection<Schema> getRegisteredWritten(final FijiColumnName columnName)
      throws IOException {
   ...
   * @throws InvalidLayoutException in case a fully qualified column in a map family is specified.
   */
  public AvroValidationPolicy getAvroValidationPolicy(final FijiColumnName column)
      throws NoSuchColumnException, InvalidLayoutException {
  ...
  private void normalizeAvroSchema(AvroSchema avroSchema, LayoutOptions options)
    ...
    switch (options.getSchemaFormat()) {
      ...
      default: throw new InternalFijiError("Unknown schema format: " + options.getSchemaFormat());
    }
  }
