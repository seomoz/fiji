...
/**
 * Base class for all Fiji command line tools.
 *
 * A command line tool, executed via {@link FijiToolLauncher}, will perform these steps when run:
 * <ol>
...
public abstract class BaseTool extends Configured implements FijiTool {
  ...
  @Flag(name="interactive", usage="Indicates whether the command may prompt the user for input.\n"
      + "\tSet --interactive=false to run the fiji command as part of a non-interactive script.")
   ...
   * uninstalling a Fiji table or instance.  If inputConfirmation is called in non-interactive
  ...
  public int toolMain(List<String> args) throws Exception {
    try {
      ...
      getPrintStream().println("Try: fiji install --fiji=" + knie.getURI());
      return FAILURE;
    }
  }
