...
import com.moz.fiji.schema.util.JvmId;
...
import com.moz.fiji.schema.zookeeper.TableUserRegistration;

/**
 * TableLayoutMonitor provides three services for users of table layouts:
 *
 *  1) it acts as a FijiTableLayout cache which is automatically refreshed when the table layout is
 *     updated.
...
public interface TableLayoutMonitor extends Closeable {
  /**
   * Get the Fiji table layout of the table.
   *
  ...
  /**
   * Update all registered LayoutConsumers with a new FijiTableLayout.
   *
   * This method is for testing purposes only.  It should not be used externally.
   *
   * @param layout the new FijiTableLayout with which to update consumers.
   * @throws IOException in case of an error updating LayoutConsumers.
   */
  void updateLayoutConsumers(FijiTableLayout layout) throws IOException;
  ...
  public static final class DefaultTableLayoutMonitor implements TableLayoutMonitor {
    ...
    private final FijiMetaTable mMetaTable;
    ...
    /**
     * A reference to the latest {@link FijiTableLayout} for the table.  Updated automatically by a
     * ZooKeeper watcher when the layout is updated.
     */
    private final AtomicReference<FijiTableLayout> mLayout = new AtomicReference<>();
     ...
     * @param schemaTable of Fiji table.
     * @param metaTable of Fiji table.
    ...
    @Override
    /** {@inheritDoc} */
    public void updateLayoutConsumers(FijiTableLayout layout) throws IOException {
      Preconditions.checkState(mState.get() == State.STARTED,
          "TableLayoutMonitor has not been started.");
    ...
    private static String generateTableUserID() {
      return String.format("%s;HBaseFijiTable@%s", JvmId.get(), TABLE_COUNTER.getAndIncrement());
    }
    ...
    private static final class InnerLayoutUpdater implements TableLayoutUpdateHandler {
      ...
      private final AtomicReference<FijiTableLayout> mLayout;
      ...
      private InnerLayoutUpdater(
          ...
          final AtomicReference<FijiTableLayout> layout,
          final FijiURI tableURI,
          final Set<LayoutConsumer> consumers,
          final FijiMetaTable metaTable,
      ...
      public void update(final String notifiedLayoutID) {
        try {
          ...
          // Registers this FijiTable in ZooKeeper as a user of the new table layout,
