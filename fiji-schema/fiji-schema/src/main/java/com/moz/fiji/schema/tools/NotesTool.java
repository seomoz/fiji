...
public final class NotesTool extends BaseTool {
  ...
  @Flag(name="target", usage="Specify the FijiURI of the target of your operation. Must include at "
  ...
  @Flag(name="key", usage="Specify an annotation key. Keys must match the pattern "
      + FijiTableAnnotator.ALLOWED_ANNOTATION_KEY_PATTERN + "\n  (use --prefix, --partial, or "
  ...
  @Flag(name="map", usage="Specify a json map of annotation key-values. Keys must match the "
      + "pattern " + FijiTableAnnotator.ALLOWED_ANNOTATION_KEY_PATTERN + ", values may be any "
      + "string.")
  ...
  private void printColumnAnnotations(
      final String key,
      final Map<FijiColumnName, String> cvs
  ) {
    for (Map.Entry<FijiColumnName, String> cvEntry : cvs.entrySet()) {
   ...
   */
  private void printColumnAnnotations(
      final Map<FijiColumnName, Map<String, String>> ckvs
  ) {
    for (Map.Entry<FijiColumnName, Map<String, String>> columnEntry : ckvs.entrySet()) {
   ...
   */
  private void printRemovedColumnAnnotations(
      final Map<FijiColumnName, Set<String>> removedKeys
  ) {
    for (Map.Entry<FijiColumnName, Set<String>> columnEntry : removedKeys.entrySet()) {
  ...
  protected void validateFlags() {
    try {
      mURI = FijiURI.newBuilder(mTargetFlag).build();
    ...
    switch (mDoMode) {
      ...
      default: throw new InternalFijiError(String.format("Unknown DoMode: %s", mDoMode));
    }
  ...
  private int set() throws IOException {
    final List<FijiColumnName> columns = mURI.getColumns();
    if (columns.isEmpty()) {
      if (mMapFlag.isEmpty()) {
  ...
  protected int run(final List<String> nonFlagArgs) throws Exception {
    final Fiji fiji = Fiji.Factory.open(mURI);
    try {
      final FijiTable table = fiji.openTable(mURI.getTable());
      try {
        ...
        try {
          switch (mDoMode) {
            ...
            case GET: return get();
            default: throw new InternalFijiError("Unknown NotesTool DoMode: " + mDoMode);
          }
        } finally {
          mAnnotator.close();
        }
