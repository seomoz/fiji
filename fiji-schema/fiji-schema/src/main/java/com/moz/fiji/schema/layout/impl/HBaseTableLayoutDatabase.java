...
/**
 * <p>Manages Fiji table layouts using a column family in an HBase table as a backing store.</p>
 *
 * <p>
 ...
 */
@ApiAudience.Private
public final class HBaseTableLayoutDatabase implements FijiTableLayoutDatabase {
  ...
  public List<FijiTableLayout> getTableLayoutVersions(String table, int numVersions)
    ...
    for (KeyValue column : result.getColumn(mFamilyBytes, QUALIFIER_LAYOUT_BYTES)) {
      layouts.add(FijiTableLayout.newLayout(decodeTableLayoutDesc(column.getValue())));
    }
    return layouts;
  }
  ...
  public NavigableMap<Long, FijiTableLayout> getTimedTableLayoutVersions(
    ...
    final NavigableMap<Long, FijiTableLayout> timedLayouts = Maps.newTreeMap();
    ...
    for (Map.Entry<Long, byte[]> timeSerieEntry : timeSerieMap.entrySet()) {
      ...
      final FijiTableLayout layout = FijiTableLayout.newLayout(decodeTableLayoutDesc(bytes));
