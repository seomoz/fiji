...
/**
 * Base class for decoders that read Fiji cells encoded using Avro.
 *
 * Fiji cells are encoded as Extension records, although for efficiency reasons, this class
 * provides a custom decoder that avoid copying bytes unnecessarily.
 *
 * @param <T> The type of the decoded cell data.
 */
@ApiAudience.Private
public abstract class AvroCellDecoder<T> implements FijiCellDecoder<T> {
  ...
  private static class SchemaHashDecoder implements SchemaDecoder {
     ...
     */
    public SchemaHashDecoder(FijiSchemaTable schemaTable) {
  ...
  private static class SchemaIdDecoder implements SchemaDecoder {
     ...
     */
    public SchemaIdDecoder(FijiSchemaTable schemaTable) {
      mSchemaTable = Preconditions.checkNotNull(schemaTable);
    }
   ...
   */
  private static Schema getSchemaForFinalColumn(FijiTableLayout layout, FijiColumnName column)
   ...
   * @throws IOException on I/O error.
   */
  private static SchemaDecoder createSchemaDecoder(FijiTableLayout layout,
   ...
   */
  private static Schema getReaderSchema(FijiTableLayout layout, BoundColumnReaderSpec spec)
      throws IOException {
    switch (spec.getColumnReaderSpec().getAvroReaderSchemaType()) {
      case DEFAULT: {
        final CellSchema cellSchema = layout.getCellSchema(spec.getColumn());
  ...
  /**
   * Initializes an abstract FijiAvroCellDecoder.
   *
   * @param cellSpec Specification of the cell encoding.
   * @throws IOException on I/O error.
   */
   ...
   * @throws IOException on I/O error.
   */
  protected AvroCellDecoder(FijiTableLayout layout, BoundColumnReaderSpec spec) throws IOException {
