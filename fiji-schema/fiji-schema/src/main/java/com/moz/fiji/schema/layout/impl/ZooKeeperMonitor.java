...
import com.moz.fiji.schema.util.ZooKeeperLock;
 ...
 *        Root ZooKeeper node for the Fiji instance with name "instance-name".
 *    </li>
 *    <li> {@code /fiji-schema/instances/[instance-name]/tables/[table-name]} :
 *        Root ZooKeeper node for the Fiji table with name "table-name" belonging to the Fiji
 ...
 * <ul>
 *   <li>
 *     {@code /fiji-schema/instances/[instance-name]/tables/[table-name]/layout} :
 ...
 *   </li>
 *   <li>
 *     {@code /fiji-schema/instances/[instance-name]/tables/[table-name]/users} :
 ...
 *   </li>
 *   <li>
 *     {@code /fiji-schema/instances/[instance-name]/tables/[table-name]/layout_update_lock} :
 ...
 * </ul>
 *
 * @deprecated use the ZooKeeper recipes in the {@link com.moz.fiji.schema.zookeeper} package instead.
 *    Will be removed in FijiSchema 2.0.
 */
...
public final class ZooKeeperMonitor implements Closeable {
  ...
  /** Root path of the ZooKeeper directory node where to write Fiji nodes. */
  private static final File ROOT_ZOOKEEPER_PATH = new File("/fiji-schema");

  /** Path of the ZooKeeper directory where instance Fiji nodes are written. */
  ...
  /**
   * Reports the ZooKeeper node path for a Fiji instance.
   *
   * @param fijiURI URI of a Fiji instance to report the ZooKeeper node path for.
   * @return the ZooKeeper node path for a Fiji instance.
   */
  public static File getInstanceDir(FijiURI fijiURI) {
    return new File(INSTANCES_ZOOKEEPER_PATH, fijiURI.getInstance());
  }
   ...
   */
  public static File getInstancePermissionsLock(FijiURI instanceURI) {
    return new File(getInstanceDir(instanceURI), "permissions_lock");
  }

  /**
   * Reports the ZooKeeper root path containing all tables in a Fiji instance.
   *
   * @param fijiURI URI of a Fiji instance to report the ZooKeeper node path for.
   * @return the ZooKeeper node path that contains all the tables in the specified Fiji instance.
   */
  public static File getInstanceTablesDir(FijiURI fijiURI) {
    return new File(getInstanceDir(fijiURI), "tables");
  }

  /**
   * Reports the ZooKeeper root path containing all users of a Fiji instance.
   *
   * @param fijiURI URI of a Fiji instance to report the ZooKeeper node path for.
   * @return the ZooKeeper node path that contains all users of the specified Fiji instance.
   */
  public static File getInstanceUsersDir(FijiURI fijiURI) {
    return new File(getInstanceDir(fijiURI), "users");
  }

  /**
   * Reports the ZooKeeper node path for a Fiji table.
   *
   * @param tableURI URI of a Fiji table to report the ZooKeeper node path for.
   * @return the ZooKeeper node path for a Fiji table.
   */
  public static File getTableDir(FijiURI tableURI) {
   ...
   *     version of the Fiji table identified by this URI.
   * @return the path of the ZooKeeper node that contains the most recent layout version of the
   *     specified Fiji table.
   */
  public static File getTableLayoutFile(FijiURI tableURI) {
   ...
   *
   * @param tableURI Reports the path of the ZooKeeper node where users of the Fiji table with this
   *     URI register themselves.
   * @return the path of the ZooKeeper node where users of a table register.
   */
  public static File getTableUsersDir(FijiURI tableURI) {
   ...
   */
  public static File getTableLayoutUpdateLock(FijiURI tableURI) {
   ...
   */
  public LayoutTracker newTableLayoutTracker(FijiURI tableURI, LayoutUpdateHandler handler) {
   ...
   */
  public TableUserRegistration newTableUserRegistration(String userID, FijiURI tableURI) {
  ...
  private static String makeZKNodeName(String userId, String layoutId) {
    try {
      ...
      throw new InternalFijiError(uee);
    }
  }
   ...
   */
  public Lock newTableLayoutUpdateLock(FijiURI tableURI) {
   ...
   */
  public UsersTracker newTableUsersTracker(FijiURI tableURI, UsersUpdateHandler handler) {
   ...
   *   {@link #newTableLayoutUpdateLock(FijiURI)}.
   * </p>
   *
   ...
   * @throws KeeperException on unrecoverable ZooKeeper error.
   */
  public void notifyNewTableLayout(FijiURI tableURI, byte[] layout, int version)
  ...
  public final class LayoutTracker implements Closeable {
     ...
     */
    private LayoutTracker(FijiURI tableURI, LayoutUpdateHandler handler) {
  ...
  public interface UsersUpdateHandler {
    ...
    /**
     * Processes an update to the list of users of the Fiji table being tracked.
     *
     * <p> If this method raises an unchecked exception, the tracking stops. </p>
     *
     * @param users Updated mapping from user ID to layout IDs of the Fiji table being tracked.
     */
    // TODO(SCHEMA-412): Notifications for ZooKeeper disconnections.
  ...
  public final class UsersTracker implements Closeable {
     ...
     */
    private UsersTracker(FijiURI tableURI, UsersUpdateHandler handler) {
      this.mTableURI = tableURI;
  ...
  public final class TableUserRegistration implements Closeable {
     ...
     */
    public TableUserRegistration(String userID, FijiURI tableURI) {
      mUserID = userID;
      mTableURI = tableURI;
    }
    ...
    private void register(String layoutID) throws IOException {
      synchronized (mMonitor) {
        try {
          ...
          LOG.debug("Registering user '{}' for Fiji table '{}' with layout ID '{}'.",
  ...
  public final class InstanceUserRegistration implements Closeable {
     ...
     * @param instanceURI Fiji uri of instance being used.
     */
    public InstanceUserRegistration(String userID, String systemVersion, FijiURI instanceURI) {
      mUserID = userID;
    ...
    public void start() throws IOException {
      synchronized (mMonitor) {
        try {
          ...
          LOG.debug("Registering user '{}' for Fiji instance '{}' with system version '{}'.",
