...
/**
 * This class represents an encapsulation of the parts of a Fiji row key,
 * suitable for translation to an {@link EntityId}.
 *
 * <p>FijiRowKeyComponents has one factory method, {@link #fromComponents(Object...)}.</p>
 *
...
public final class FijiRowKeyComponents implements Comparable<FijiRowKeyComponents> {
   ...
   */
  private FijiRowKeyComponents(Object[] components) {
    mComponents = components;
  }

  /**
   * Creates a FijiRowKeyComponents from components.
   *
   * @param components the components of the row key.
   * @return a FijiRowKeyComponents
   */
  public static FijiRowKeyComponents fromComponents(Object... components) {
    ...
    // Some of these checks will be redundant when the FijiRowKeyComponents is converted
    ...
    if (components[0] instanceof byte[]) {
      ...
      return new FijiRowKeyComponents(new Object[]{Arrays.copyOf(original, original.length)});
    } else {
      ...
      for (int i = 0; i < components.length; i++) {
        if (seenNull) {
          Preconditions.checkArgument(
              components[i] == null,
              "Fiji Row Keys cannot contain have a non-null component after a null component");
        } else {
    ...
    return new FijiRowKeyComponents(Arrays.copyOf(components, components.length));
  }

  /**
   * Creates a FijiRowKeyComponents from a list of components.
   *
   * @param componentsList A List&lt;Object&gt; of the components of the row key.
   * @return a FijiRowKeyComponents
   */
  public static FijiRowKeyComponents fromComponentsList(List<Object> componentsList) {
    return fromComponents(componentsList.toArray());
  }

  /**
   * Gets an EntityId for this FijiRowKeyComponents appropriate to the given table.
   *
   * @param table the FijiTable to generate an EntityId. The table's row key format must be
   *     compatible with the number and type of the components.
   * @return an EntityId appropriate to the table.
   */
  public EntityId getEntityIdForTable(FijiTable table) {
  ...
  public boolean equals(Object obj) {
    ...
    final FijiRowKeyComponents krkc = (FijiRowKeyComponents)obj;
   ...
   * {@inheritDoc}
   *
   * *Note* the ordering of FijiRowKeyComponents is not representative of the scan order of rows in
   * a Fiji table. Hashing is not taken into account when comparing {@link FijiRowKeyComponents}.
   *
   * Only {@link FijiRowKeyComponents} from the same table may be compared. The compared row key
   ...
   */
  @Override
  public int compareTo(FijiRowKeyComponents other) {
    ...
    if (size1 != size2) {
      throw new IllegalArgumentException(
          String.format(
              "Can not compare FijiRowKeyComponents with different numbers of components."
    ...
    for (int i = 0; i < size1; i++) {
      ...
      try {
        ...
        throw new IllegalArgumentException(
            String.format(
                "Can not compare FijiRowKeyComponents with different component types."
