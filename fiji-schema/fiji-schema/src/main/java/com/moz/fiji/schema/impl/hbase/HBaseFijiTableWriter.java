...
/**
 * Makes modifications to a Fiji table by sending requests directly to HBase from the local client.
 *
 ...
 */
@ApiAudience.Private
public final class HBaseFijiTableWriter implements FijiTableWriter {
  private static final Logger LOG = LoggerFactory.getLogger(HBaseFijiTableWriter.class);
  ...
  private final class InnerLayoutUpdater implements LayoutConsumer {
    ...
    public void update(final FijiTableLayout layout) throws IOException {
      ...
      if (mWriterLayoutCapsule != null) {
        LOG.debug(
            "Updating layout used by FijiTableWriter: {} for table: {} from version: {} to: {}",
            this,
            mTable.getURI(),
            mWriterLayoutCapsule.getLayout().getDesc().getLayoutId(),
            layout.getDesc().getLayoutId());
      } else {
        LOG.debug("Initializing FijiTableWriter: {} for table: {} with table layout version: {}",
            this,
            mTable.getURI(),
            layout.getDesc().getLayoutId());
      }
   ...
   */
  public HBaseFijiTableWriter(HBaseFijiTable table) throws IOException {
    mTable = table;
    ...
    Preconditions.checkState(mWriterLayoutCapsule != null,
        "FijiTableWriter for table: %s failed to initialize.", mTable.getURI());
    ...
    Preconditions.checkState(oldState == State.UNINITIALIZED,
        "Cannot open FijiTableWriter instance in state %s.", oldState);
  }
  ...
  public <T> void put(EntityId entityId, String family, String qualifier, long timestamp, T value)
    ...
    Preconditions.checkState(state == State.OPEN,
        "Cannot put cell to FijiTableWriter instance %s in state %s.", this, state);
  ...
  /** {@inheritDoc} */
  @Override
  public FijiCell<Long> increment(EntityId entityId, String family, String qualifier, long amount)
      throws IOException {
    final State state = mState.get();
    Preconditions.checkState(state == State.OPEN,
        "Cannot increment cell to FijiTableWriter instance %s in state %s.", this, state);

    verifyIsCounter(family, qualifier);

    // Translate the Fiji column name to an HBase column name.
  ...
  public void deleteRow(EntityId entityId, long upToTimestamp) throws IOException {
    final State state = mState.get();
    Preconditions.checkState(state == State.OPEN,
        "Cannot delete row while FijiTableWriter %s is in state %s.", this, state);
  ...
  public void deleteFamily(EntityId entityId, String family, long upToTimestamp)
    ...
    final State state = mState.get();
    Preconditions.checkState(state == State.OPEN,
        "Cannot delete family while FijiTableWriter %s is in state %s.", this, state);
    ...
    final HBaseColumnName hbaseColumnName = capsule.getColumnNameTranslator()
        .toHBaseColumnName(FijiColumnName.create(family));
  ...
  private void deleteGroupFamily(
    ...
    final State state = mState.get();
    Preconditions.checkState(state == State.OPEN,
        "Cannot delete family group while FijiTableWriter %s is in state %s.", this, state);
    ...
    for (ColumnLayout columnLayout : familyLayout.getColumnMap().values()) {
      ...
      final FijiColumnName column = FijiColumnName.create(familyName, qualifier);
      final HBaseColumnName hbaseColumnName =
  ...
  private void deleteMapFamily(EntityId entityId, FamilyLayout familyLayout, long upToTimestamp)
    ...
    final State state = mState.get();
    Preconditions.checkState(state == State.OPEN,
        "Cannot delete map family while FijiTableWriter %s is in state %s.", this, state);
    final String familyName = familyLayout.getName();
    final HBaseColumnName hbaseColumnName = mWriterLayoutCapsule.getColumnNameTranslator()
        .toHBaseColumnName(FijiColumnName.create(familyName));
  ...
  public void deleteColumn(EntityId entityId, String family, String qualifier, long upToTimestamp)
    ...
    final State state = mState.get();
    Preconditions.checkState(state == State.OPEN,
        "Cannot delete column while FijiTableWriter %s is in state %s.", this, state);
  ...
  public void deleteCell(EntityId entityId, String family, String qualifier, long timestamp)
    ...
    final State state = mState.get();
    Preconditions.checkState(state == State.OPEN,
        "Cannot delete cell while FijiTableWriter %s is in state %s.", this, state);
  ...
  public void close() throws IOException {
    ...
    Preconditions.checkState(oldState == State.OPEN,
        "Cannot close FijiTableWriter instance %s in state %s.", this, oldState);
  ...
  public String toString() {
    return Objects.toStringHelper(HBaseFijiTableWriter.class)
