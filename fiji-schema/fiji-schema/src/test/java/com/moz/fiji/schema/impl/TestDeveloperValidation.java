 ...
 */
public class TestDeveloperValidation extends FijiClientTest {
  ...
  public static final String LAYOUT_DEVELOPER =
      "com.moz.fiji/schema/layout/layout-developer.json";
  ...
  public void testNoReaderSchema() throws Exception {
    ...
    fiji.createTable(FijiTableLayouts.getLayout(LAYOUT_DEVELOPER));
    final FijiTable table = fiji.openTable("dev");
    try {
      ...
      final List<AvroSchema> expectedIds = Lists.newArrayList(
          AvroSchema.newBuilder()
              .setUid(fiji.getSchemaTable().getOrCreateSchemaId(SCHEMA_LONG))
              .build(),
          AvroSchema.newBuilder()
              .setUid(fiji.getSchemaTable().getOrCreateSchemaId(SCHEMA_STRING))
              .build(),
          AvroSchema.newBuilder()
              .setUid(fiji.getSchemaTable().getOrCreateSchemaId(TestRecord1.SCHEMA$))
              .build());

      final List<AvroSchema> writerSchemaIds =
          table.getLayout().getCellSchema(FijiColumnName.create("info:user_id")).getWriters();
      Assert.assertEquals(expectedIds, writerSchemaIds);

      final List<AvroSchema> writtenSchemaIds =
          table.getLayout().getCellSchema(FijiColumnName.create("info:user_id")).getWritten();
  ...
  public void testReaderSchemaLong() throws Exception {
    final Fiji fiji = getFiji();
    final TableLayoutDesc desc = FijiTableLayouts.getLayout(LAYOUT_DEVELOPER);
  ...
  public void testReaderSchemaTestRecord2() throws Exception {
    ...
    cellSchema.setReaders(Lists.newArrayList(
        AvroSchema.newBuilder()
            .setUid(fiji.getSchemaTable().getOrCreateSchemaId(INT_TEXT_RECORD1))
            .build()));
