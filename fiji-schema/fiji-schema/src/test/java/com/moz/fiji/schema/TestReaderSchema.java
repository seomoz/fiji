...
import com.moz.fiji.schema.avro.TestRecord2;
import com.moz.fiji.schema.avro.TestRecord3;
import com.moz.fiji.schema.layout.CellSpec;
import com.moz.fiji.schema.layout.FijiTableLayouts;

public class TestReaderSchema extends FijiClientTest {
  private static final Logger LOG = LoggerFactory.getLogger(TestReaderSchema.class);

  private static final String TABLE_NAME = "table";

  /** FijiTable used for the test (named TABLE_NAME). */
  private FijiTable mTable;

  /** Requests all columns in family "family". */
  private static final FijiDataRequest DATA_REQUEST = FijiDataRequest.builder()
      .addColumns(ColumnsDef.create().withMaxVersions(HConstants.ALL_VERSIONS).addFamily("family"))
      .build();

  @Before
  public final void setupTestHBaseFijiRowData() throws Exception {
    getFiji().createTable(FijiTableLayouts.getLayout(FijiTableLayouts.READER_SCHEMA_TEST));
  ...
  public void testDecodeEmptyAsRecord1() throws Exception {
    ...
    final FijiColumnName colEmpty = FijiColumnName.create("family", "empty");
    final Map<FijiColumnName, CellSpec> overrides =
        ImmutableMap.<FijiColumnName, CellSpec>builder()
            ...
            .build();
    final FijiTableReader reader = mTable.getReaderFactory().openTableReader(overrides);
    try {
      final FijiRowData row = reader.get(eid, DATA_REQUEST);
  ...
  public void testDecodeRecord1AsRecord2() throws Exception {
    ...
    final FijiColumnName colEmpty = FijiColumnName.create("family", "record1");
    final Map<FijiColumnName, CellSpec> overrides =
        ImmutableMap.<FijiColumnName, CellSpec>builder()
            .put(colEmpty, mTable.getLayout().getCellSpec(colEmpty)
  ...
  public void testDecodeRecord2AsRecord1() throws Exception {
    ...
    final FijiColumnName colEmpty = FijiColumnName.create("family", "record2");
    final Map<FijiColumnName, CellSpec> overrides =
        ImmutableMap.<FijiColumnName, CellSpec>builder()
            .put(colEmpty, mTable.getLayout().getCellSpec(colEmpty)
                .setReaderSchema(TestRecord1.SCHEMA$))
  ...
  public void testDecodeIntAsLong() throws Exception {
    ...
    final FijiColumnName colEmpty = FijiColumnName.create("family", "integer");
    final Map<FijiColumnName, CellSpec> overrides =
        ImmutableMap.<FijiColumnName, CellSpec>builder()
            .put(colEmpty, mTable.getLayout().getCellSpec(colEmpty)
  ...
  public void testDecodeIntAsRecord2Fails() throws Exception {
    ...
    try {
      ...
      try {
        ...
        // Expected
        assertTrue(ate.getMessage(),
            ate.getMessage().contains("Found int, expecting com.moz.fiji.schema.avro.TestRecord2"));
      }
  ...
  public void testDecodeWithWriterSchema() throws Exception {
    ...
    final FijiColumnName colEmpty = FijiColumnName.create("family", "records");
    final Map<FijiColumnName, CellSpec> overrides =
        ImmutableMap.<FijiColumnName, CellSpec>builder()
            .put(colEmpty, mTable.getLayout().getCellSpec(colEmpty)
                .setUseWriterSchema())
