...
import com.moz.fiji.schema.impl.hbase.HBaseTableKeyValueDatabase;

/**
 * This class tests that HBaseTableLayoutDatabase is correctly writing and reading from HBase
 * when performing its operations.
 */
public class TestHBaseTableKeyValueDatabase extends FijiClientTest {
  ...
  public final void setupHBaseTable() throws IOException {
    ...
    mConf = super.getConf();
    mHBaseAdmin = factory.getHBaseAdminFactory(getFiji().getURI()).create(mConf);
    // Create an HBase table.
    HTableDescriptor tableDescriptor = new HTableDescriptor(TABLE_NAME);
    tableDescriptor.addFamily(new HColumnDescriptor(FAMILY_NAME));
    mHBaseAdmin.createTable(tableDescriptor);

    mTable = factory.getHTableInterfaceFactory(getFiji().getURI()).create(mConf, TABLE_NAME);
