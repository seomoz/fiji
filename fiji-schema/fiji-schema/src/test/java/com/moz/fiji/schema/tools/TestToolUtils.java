...
public class TestToolUtils {
  ...
  private final FijiTableLayout mFormattedLayout =
      FijiTableLayouts.getTableLayout(FijiTableLayouts.FORMATTED_RKF);

  private final FijiTableLayout mRawLayout =
      FijiTableLayouts.getTableLayout(FijiTableLayouts.SQOOP_EXPORT_SAMPLING_UNHASHED_TEST);

  private final FijiTableLayout mHashedLayout =
      FijiTableLayouts.getTableLayout(FijiTableLayouts.SQOOP_EXPORT_SAMPLING_HASHED_TEST);

  private final FijiTableLayout mHashPrefixedLayout =
      FijiTableLayouts.getTableLayout(FijiTableLayouts.HASH_PREFIXED_RKF);
  ...
  public void testTooLargeInt() throws Exception {
    try {
      ...
      assertEquals("Invalid type for component 2147483648 at index 3 in fijiRowKey",
          eie.getMessage());
    }
  }
