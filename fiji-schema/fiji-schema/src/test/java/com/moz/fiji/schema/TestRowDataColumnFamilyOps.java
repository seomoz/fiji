 ...
 */
public class TestRowDataColumnFamilyOps extends FijiClientTest {
  private static final Logger LOG = LoggerFactory.getLogger(TestRowDataColumnFamilyOps.class);
  private FijiRowData mRow1;

  @Before
  public void setupInstance() throws Exception {
   final FijiTableLayout layout =
      FijiTableLayout.newLayout(FijiTableLayouts.getLayout(
          FijiTableLayouts.TWO_COLUMN_DIFFERENT_TYPES));
  ...
  public void testGetMostRecentCells() throws IOException {
     ...
     assertTrue("Row does not contain column [family:column2].",
         mRow1.containsColumn("family", "column2"));
     try {
     final NavigableMap<String, FijiCell<CharSequence>> stringGet =
         mRow1.getMostRecentCells("family");
     final NavigableMap<String, FijiCell<Integer>> intGet = mRow1.getMostRecentCells("family");
     assertEquals("I am not a string! () for the stringGet.", "I am a String",
  ...
  public void testGetCells() throws IOException {
     ...
     assertTrue("Row does not contain column [family:column2].",
         mRow1.containsColumn("family", "column2"));
     try {
     final NavigableMap<String, NavigableMap<Long, FijiCell<CharSequence>>> stringGet =
         mRow1.getCells("family");
     final NavigableMap<String, NavigableMap<Long, FijiCell<Integer>>> intGet =
         mRow1.getCells("family");
     assertEquals("I am not a string! () for the stringGet.", "I am a String",
  ...
  public void testIterator() throws IOException {
     ...
     assertTrue("Row does not contain column [family:column2].",
         mRow1.containsColumn("family", "column2"));
     try {
       final Iterator<FijiCell<CharSequence>> stringGet = mRow1.iterator("family");
       fail("Didn't throw an exception!");
     } catch (IllegalStateException ex) {
  ...
  public void testGetPager() throws IOException {
    ...
    try {
      ...
      try {
        ...
        final FijiRowData row1 = reader.get(table.getEntityId("row1"),
                  dataRequest);
        try {
          FijiPager pager = row1.getPager("family");
          pager.close();
          fail("Didn't throw an exception!");
        } catch (IllegalStateException ex) {
