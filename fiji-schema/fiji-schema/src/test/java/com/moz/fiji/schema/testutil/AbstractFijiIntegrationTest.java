...
/**
 * A base class for all Fiji integration tests.
 *
 * <p>
 ...
 */
public abstract class AbstractFijiIntegrationTest {
  private static final Logger LOG = LoggerFactory.getLogger(AbstractFijiIntegrationTest.class);

  public static final String ADD_CLASSPATH_TO_JOB_DCACHE_PROPERTY =
      "com.moz.fiji.mapreduce.add.classpath.to.job.dcache";
   ...
   *   mvn clean verify -Dfiji.test.cluster.uri=fiji://localhost:2181
   */
  private static final String BASE_TEST_URI_PROPERTY = "fiji.test.cluster.uri";
  ...
  public static final String CLEANUP_AFTER_TEST_PROPERTY =
      "com.moz.fiji.schema.test.cleanup.after.test";

  /**
   * Whether to remove temporary fiji instances after testing is over. This defaults to true
  ...
  private static final String HBASE_MAVEN_PLUGIN_URI = "fiji://.env/";

  private static FijiURI mHBaseURI;
  ...
  /** A thread that creates Fiji instances for use by tests. */
  private static FijiCreationThread mCreationThread;
  /** A thread that removes Fiji instances no longer in use by tests. */
  private static FijiDeletionThread mDeletionThread;
   ...
   */
  protected static FijiURI getHBaseURI() {
    if (TEST_CLUSTER_URI != null) {
      return FijiURI.newBuilder(TEST_CLUSTER_URI).build();
    } else {
      return FijiURI.newBuilder(HBASE_MAVEN_PLUGIN_URI).build();
    }
  }
  ...
  public static void setupManagementThreads() throws Exception {
    ...
    synchronized (THREAD_MANAGEMENT_LOCK) {
      ...
      if (null == mCreationThread) {
        LOG.info("Starting Fiji instance creation thread.");
        mCreationThread = new FijiCreationThread(mHBaseURI);
        mCreationThread.start();
        if (CLEANUP_AFTER_TEST) {
          LOG.info("Starting Fiji instance deletion thread.");
          mDeletionThread = new FijiDeletionThread();
  ...
  public static void teardownManagementThreads() {
    synchronized (THREAD_MANAGEMENT_LOCK) {
      ...
      if (RUNNING_TEST_SEMAPHORE.availablePermits() == 0) {
        ...
        mCreationThread.stopFijiCreation();
        FijiURI unusedFijiURI = mCreationThread.getFijiForCleanup();
        while (CLEANUP_AFTER_TEST && (null != unusedFijiURI)) {
          try {
            mDeletionThread.destroyFiji(unusedFijiURI);
          } catch (InterruptedException exn) {
            LOG.error("Failed to put Fiji instance '{}' into the deletion queue: {}",
                unusedFijiURI, exn.getMessage());
            continue;
          }
          unusedFijiURI = mCreationThread.getFijiForCleanup();
        }
  ...
  public final void setupFijiIntegrationTest() throws Exception {
    ...
    mFijiURI = mCreationThread.getFreshFiji();
    mHelper = new IntegrationHelper(mConf);
  }

  @After
  public final void teardownFijiIntegrationTest() throws Exception {
    // Schedule the Fiji instance for asynchronous deletion.
    if (CLEANUP_AFTER_TEST && (null != mFijiURI)) {
      mDeletionThread.destroyFiji(mFijiURI);
    }
  ...
  /**
   * Thread that creates Fiji instances for use by integration tests.
   */
  private static final class FijiCreationThread extends Thread {
    /** Maintain a pool of this many fresh Fiji instances ready to go. */
    ...
    /** Bounded producer/consumer queue that holds the names of new Fiji instances. */
    private final LinkedBlockingQueue<FijiURI> mFijiQueue =
        new LinkedBlockingQueue<FijiURI>(INSTANCE_POOL_SIZE);

    private final FijiURI mHBaseURI;

    /** Denotes whether this thread should continue to create Fiji instances. */
    private boolean mActive = true;


    private FijiCreationThread(FijiURI hbaseURI) {
      setName("FijiCreationThread");
      setDaemon(true);
      mHBaseURI = hbaseURI;
    }

    /**
     * Get a new Fiji instance from the pool. This may block if we're using Fiji
     * instances in tests too fast.
     *
     * @return a FijiURI identifying an unused Fiji instance.
     * @throws InterruptedException if the blocking call is interrupted.
     */
    public FijiURI getFreshFiji() throws InterruptedException {
      return mFijiQueue.take();
    }

    /**
     * Get a remaining Fiji instance from the pool for cleaning up. This may block if we're out
     * of Fiji instances and there are still instances being created
     *
     * Package private as this should only be use for cleanup
     *
     * @return a FijiURI identifying an unused Fiji instance for cleaning up
     */
    FijiURI getFijiForCleanup() {
      if (!mFijiQueue.isEmpty()) {
        return mFijiQueue.poll();
      }
    ...
    /**
     * Stops the creation of new Fiji instances.  The creation thread will finish creating any
     * instances that are still in progress.
     */
    public void stopFijiCreation() {
      mActive = false;
    }
    ...
    public void run() {
      synchronized (THREAD_CREATION_ACTIVE_LOCK) {
        while (mActive) {
          // Create a new Fiji instance.
          final String instanceName = UUID.randomUUID().toString().replaceAll("-", "_");
          final Configuration conf = HBaseConfiguration.create();
          try {
            final FijiURI fijiURI =
                FijiURI.newBuilder(mHBaseURI).withInstanceName(instanceName).build();
            FijiInstaller.get().install(fijiURI, conf);

            // This blocks if the queue is full:
            mFijiQueue.put(fijiURI);

          } catch (Exception exn) {
            LOG.error(String.format("Exception while installing Fiji instance [%s]: %s\n%s",
  ...
  /**
   * Thread that destroys Fiji instances used by integration tests.
   */
  private static final class FijiDeletionThread extends Thread {
    /** Unbounded producer/consumer queue that holds Fiji instances to destroy. */
    private LinkedBlockingQueue<FijiURI> mFijiQueue;
    ...
    private FijiDeletionThread() {
      setName("FijiDeletionThread");
      setDaemon(true);
      mFijiQueue = new LinkedBlockingQueue<FijiURI>();
      mActive = true;
    }

    /**
     * Add a Fiji instance to the list of Fiji instances to be destroyed.
     * The instance will be destroyed asynchronously.
     *
     * @param fijiURI a String identifying a Fiji instance we're done with.
     * @throws InterruptedException if the blocking call is interrupted.
     */
    public void destroyFiji(FijiURI fijiURI) throws InterruptedException {
      mFijiQueue.put(fijiURI);
    }
    ...
    public void run() {
      ...
      synchronized (THREAD_DELETION_ACTIVE_LOCK) {
        while (!mFijiQueue.isEmpty() || mActive) {
          FijiURI instanceURI = null;

          try {
            instanceURI = mFijiQueue.take();
          ...
          if (null == instanceURI) {
            LOG.warn("Unexpected null Fiji instance after take() in destroy thread");
            continue;
          }

          try {
            intHelper.uninstallFiji(instanceURI);
          } catch (Exception e) {
            LOG.error("Could not destroy Fiji instance [" + instanceURI.toString() + "]: "
                + e.getMessage());
          }
        }
      }
