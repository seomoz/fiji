...
import com.moz.fiji.schema.avro.HashType;
...
public class TestFijiTableLayout {
  private static final Logger LOG = LoggerFactory.getLogger(TestFijiTableLayout.class);
  ...
  // This is an admissable RowKeyFormat2 object, but only because FijiTableLayout
  ...
  public void testLayoutIDs() throws Exception {
    ...
    final TableLayoutDesc descV2 = TableLayoutDesc.newBuilder(desc)
        ...
        .build();
    final FijiTableLayout layoutV2 = FijiTableLayout.createUpdatedLayout(descV2, layout);
  ...
  public void testDeprecatedVersion() throws Exception {
    final TableLayoutDesc validDesc = TableLayoutDesc.newBuilder()
        ...
        .build();
    try {
      FijiTableLayout validLayout = FijiTableLayout.newLayout(validDesc);
    } catch (InvalidLayoutException ile) {
      fail("Deprecated version 'fiji-1.0' should be valid.");
    }
    ...
    final TableLayoutDesc invalidDesc = TableLayoutDesc.newBuilder()
        ...
        .build();
    try {
      FijiTableLayout invalidLayout = FijiTableLayout.newLayout(invalidDesc);
      fail("Layout version fiji-1.1 should be invalid.");
    } catch (InvalidLayoutException ile) {
      // Expected.
    }
  }
  ...
  public void testMapFamilyLayoutWithNoReference() throws Exception {
    ...
    assertEquals(SchemaStorage.HASH, layout.getCellFormat(FijiColumnName.create("family_name")));
    assertEquals(Schema.Type.INT, layout.getSchema(FijiColumnName.create("family_name")).getType());
  }
  ...
  public void testFromJsonAndToJson() throws Exception {
    final FijiTableLayout layout =
        FijiTableLayout.createFromEffectiveJsonResource(
          "/com.moz.fiji/schema/layout/full-featured-layout.json");

    final String effectiveLayout = ToJson.toJsonString(layout.getDesc());
    LOG.info(effectiveLayout);
    final FijiTableLayout reparsed =
        FijiTableLayout.createFromEffectiveJson(
  ...
  public void testDuplicateFamilyName() throws Exception {
    ...
    final TableLayoutDesc desc = TableLayoutDesc.newBuilder()
        ...
        .build();
    try {
      FijiTableLayout.newLayout(desc);
  ...
  public void testFinalColumnSchema() throws Exception {
    ...
    assertEquals(
        SchemaStorage.FINAL,
        layout.getCellSchema(FijiColumnName.create("family_name", "column_name")).getStorage());
  }
  ...
  @Test
  public void testFijiTableHashSize() throws Exception {
    // default hash size for RowKeyFormat2 is 16
    assertEquals(16, FijiTableLayout.getHashSize(makeHashPrefixedRowKeyFormat()));
    // default hash size for RowKeyFormat is 0
    assertEquals(0, FijiTableLayout.getHashSize(makeHashPrefixedRKF1()));
  }
  ...
  public void testSuppressMaterializationRKF() throws InvalidLayoutException {
    final TableLayoutDesc desc = TableLayoutDesc.newBuilder()
        ...
        .setVersion(TABLE_LAYOUT_VERSION)
        .build();
    final FijiTableLayout ktl = FijiTableLayout.newLayout(desc);
  }
   ...
   * FijiTableLayout#validateCellSchema.
