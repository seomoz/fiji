...
public class TestHBaseDataRequestAdapter extends FijiClientTest {
  ...
  public void setupLayout() throws Exception {
    ...
    getFiji().createTable(tableLayout.getDesc());

    mTableLayout = getFiji().getMetaTable().getTableLayout("user");
  ...
  public void testDataRequestToScan() throws IOException {
    ...
    HBaseColumnName hPurchasesColumn = mColumnNameTranslator.toHBaseColumnName(
        FijiColumnName.create("purchases"));
  ...
  public void testDataRequestToScanEmpty() throws IOException {
    FijiDataRequest request = FijiDataRequest.builder().build();
    HBaseDataRequestAdapter hbaseDataRequest =
        new HBaseDataRequestAdapter(request, mColumnNameTranslator);
  ...
  public void testMaxVersionsEqualsOne() throws Exception {
    ...
    final FijiTable table = fiji.openTable("row_data_test_table");
    try {
      ...
      try {
        final FijiDataRequest dataRequest = FijiDataRequest.builder()
            ...
            .addColumns(ColumnsDef.create()
                ...
                .add("family", "qual2"))
            .build();
        final FijiRowData row = reader.get(table.getEntityId("row0"), dataRequest);
