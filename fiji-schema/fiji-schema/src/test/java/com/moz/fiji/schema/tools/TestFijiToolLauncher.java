...
public class TestFijiToolLauncher {
  /**
   * Tests that we can lookup a tool where we expect.
   */
  @Test
  public void testGetToolName() {
    FijiTool tool = new FijiToolLauncher().getToolForName("ls");
  ...
  public void testGetMissingTool() {
    FijiTool tool = new FijiToolLauncher().getToolForName("this-tool/can't1`exist");
    assertNull(tool);
  }
  ...
  public void testSetsOptionsParsedFlag() throws Exception {
    ...
    FijiTool tool = launcher.getToolForName("help");
