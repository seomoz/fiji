...
public class TestHBaseFijiTableAnnotator extends FijiClientTest {
  ...
  private static final FijiColumnName INFONAME = FijiColumnName.create("info:name");
  private static final FijiColumnName INFOEMAIL = FijiColumnName.create("info:email");
  private static final String INFO = "info";

  private HBaseFijiTable mTable = null;
  ...
  public void setup() throws IOException {
    new InstanceBuilder(getFiji())
        ...
        .build();
    mTable = HBaseFijiTable.downcast(getFiji().openTable("user"));
  ...
  public void testRegex() {
    ...
    assertTrue(HBaseFijiTableAnnotator.isValidAnnotationKey(valid));

    final List<String> invalidStrings =
        Lists.newArrayList("abc?", "abc.", "a$", "a!", "a#", "a-", "");
    for (String invalid : invalidStrings) {
      assertFalse(HBaseFijiTableAnnotator.isValidAnnotationKey(invalid));
    }
  }
  ...
  public void testSet() throws IOException {
    ...
    assertEquals(VALUE, Bytes.toString(getFiji().getMetaTable().getValue(mTable.getName(),
        HBaseFijiTableAnnotator.getMetaTableKey(mTable, INFONAME, KEY))));
    assertEquals(VALUE, Bytes.toString(getFiji().getMetaTable().getValue(mTable.getName(),
        HBaseFijiTableAnnotator.getMetaTableKey(
            mTable, INFOEMAIL, KEY))));
    assertEquals(VALUE2, Bytes.toString(getFiji().getMetaTable().getValue(mTable.getName(),
        HBaseFijiTableAnnotator.getMetaTableKey(
            mTable, INFOEMAIL, KEY2))));
    assertEquals(VALUE, Bytes.toString(getFiji().getMetaTable().getValue(mTable.getName(),
        HBaseFijiTableAnnotator.getMetaTableKey(KEY))));
    assertEquals(VALUE2, Bytes.toString(getFiji().getMetaTable().getValue(mTable.getName(),
        HBaseFijiTableAnnotator.getMetaTableKey(KEY2))));
    assertEquals(VALUE3, Bytes.toString(getFiji().getMetaTable().getValue(mTable.getName(),
        HBaseFijiTableAnnotator.getMetaTableKey(KEY3))));
  }
  ...
  public void testRemove() throws IOException {
    ...
    try {
      getFiji()
          .getMetaTable()
          .getValue(mTable.getName(), HBaseFijiTableAnnotator.getMetaTableKey(KEY));
      ...
      assertEquals(String.format("Could not find any values associated with table %s and key %s",
          mTable.getName(), HBaseFijiTableAnnotator.getMetaTableKey(KEY)), ioe.getMessage());
    }
    ...
    try {
      getFiji().getMetaTable().getValue(
          mTable.getName(), HBaseFijiTableAnnotator.getMetaTableKey(KEY2));
      fail("Should have thrown IOException for missing key.");
    } catch (IOException ioe) {
      assertEquals(String.format("Could not find any values associated with table %s and key %s",
          mTable.getName(), HBaseFijiTableAnnotator.getMetaTableKey(KEY2)), ioe.getMessage());
    }
    ...
    try {
      getFiji().getMetaTable().getValue(
          mTable.getName(), HBaseFijiTableAnnotator.getMetaTableKey(KEY));
      fail("Should have thrown IOException for missing key.");
    } catch (IOException ioe) {
      assertEquals(String.format("Could not find any values associated with table %s and key %s",
          mTable.getName(), HBaseFijiTableAnnotator.getMetaTableKey(KEY)), ioe.getMessage());
    ...
    try {
      getFiji().getMetaTable().getValue(mTable.getName(),
          HBaseFijiTableAnnotator.getMetaTableKey(mTable, INFONAME, KEY));
      fail("Should have thrown IOException for missing key.");
    } catch (IOException ioe) {
      assertEquals(String.format("Could not find any values associated with table %s and key %s",
          mTable.getName(), HBaseFijiTableAnnotator.getMetaTableKey(
    ...
    try {
      getFiji().getMetaTable().getValue(mTable.getName(), HBaseFijiTableAnnotator.getMetaTableKey(
    ...
    try {
      getFiji().getMetaTable().getValue(
          mTable.getName(), HBaseFijiTableAnnotator.getMetaTableKey(mTable, INFONAME, KEY2));
      fail("Should have thrown IOException for missing key.");
    } catch (IOException ioe) {
      assertEquals(String.format("Could not find any values associated with table %s and key %s",
          mTable.getName(), HBaseFijiTableAnnotator.getMetaTableKey(
    ...
    try {
      getFiji().getMetaTable().getValue(
          mTable.getName(), HBaseFijiTableAnnotator.getMetaTableKey(mTable, INFONAME, KEY));
      fail("Should have thrown IOException for missing key.");
    } catch (IOException ioe) {
      assertEquals(String.format(
          "Could not find any values associated with table %s and key %s",
