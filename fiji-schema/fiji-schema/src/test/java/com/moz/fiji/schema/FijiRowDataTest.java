...
/**
 * Tests for the {@link FijiRowData} API. Implementations of {@code FijiRowData} should provide a
 * concrete implementation for this test suite.
 */
public abstract class FijiRowDataTest extends FijiClientTest {
  ...
  @Before
  public final void setupFijiRowDataTest() throws Exception {
    mFiji = getFiji();
  ...
  @After
  public final void teardownFijiRowDataTest() throws Exception {
    mWriter.close();
    mReader.close();
    mTable.release();
    mTable = null;
  ...
  /**
   * Create a {@link FijiRowData} for the given table, reader, entity id, and data request.
   * Concrete implementations of {@code FijiRowData} should implement this test class and this
   * {@code FijiRowData} generator.
   *
   * @param table The table from which to create the {@link FijiRowData}.
   ...
   * @return The requested Fiji row data object.
   * @throws IOException On error creating the Fiji row data.
   */
  public abstract FijiRowData getRowData(
  ...
  public void testEntityId() throws IOException {
    ...
    final FijiRowData data = getRowData(mTable, mReader, eid, FijiDataRequest.empty());
  ...
  public void testReadInts() throws IOException {
    ...
    final FijiRowData data = getRowData(
        mTable,
        mReader, eid, FijiDataRequest.create(FAMILY, QUALIFIER_3));
  ...
  public void testGetReaderSchemaLayout13() throws Exception {
    mFiji.createTable(FijiTableLayouts.getLayout(TEST_LAYOUT_V1_3));
    final FijiTable table = mFiji.openTable("table");
    try {
      final FijiTableReader reader = table.getReaderFactory().openTableReader();
      try {
        final FijiRowData row = reader.get(table.getEntityId("eid"), FijiDataRequest.empty());
  ...
  public void testReadWithMaxVersions() throws IOException {
    ...
    final FijiRowData data = getRowData(mTable, mReader, eid, dataRequest);
  ...
  public void testReadMapFamilyTypes() throws IOException {
    ...
    final FijiDataRequest dataRequest = FijiDataRequest.create(MAP_FAMILY);
  ...
  public void testReadSpecificTypes() throws IOException {
    ...
    final FijiDataRequest dataRequest = FijiDataRequest.create(FAMILY, QUALIFIER_NODE_0);
    final FijiRowData data = getRowData(mTable, mReader, eid, dataRequest);
  ...
  public void testContainsColumn() throws Exception {
    ...
    final FijiRowData data = getRowData(
        mTable,
        mReader, eid, FijiDataRequest.create(FAMILY, QUALIFIER_0));
  ...
  public void testIterator() throws IOException {
    ...
    assertTrue(data.containsColumn(FAMILY));
    assertTrue(data.containsColumn(FAMILY, QUALIFIER_0));
    final Iterator<FijiCell<CharSequence>> cells = data.iterator(FAMILY, QUALIFIER_0);
    ...
    try {
      ...
      fail("HBaseFijiRowData.iterator() should fail on columns with no data request.");
  ...
  public void testCellIterator() throws IOException {
    ...
    final FijiDataRequest request = FijiDataRequest.create(FAMILY);
    final FijiRowData data = getRowData(mTable, mReader, eid, request);

    final Iterator<FijiCell<String>> iter = data.iterator(FAMILY, QUALIFIER_1);
  ...
  public void testCellIteratorReversed() throws IOException {
    ...
    final Iterator<FijiCell<String>> iter = data.iterator(FAMILY, QUALIFIER_0);
  ...
  public void testIteratorMapFamilyTypes() throws IOException {
    ...
    final FijiRowData data = getRowData(mTable, mReader, eid, dataRequest);
    final Iterator<FijiCell<Integer>> cells = data.iterator("map");
    assertTrue(cells.hasNext());
    ...
    final Iterator<FijiCell<Integer>> cellsKey1 = data.iterator(MAP_FAMILY, KEY_1);
  ...
  public void testIteratorMapFamilyMaxVersionsTypes() throws IOException {
    ...
    final FijiRowData data = getRowData(mTable, mReader, eid, dataRequest);
    final Iterator<FijiCell<Integer>> cells = data.iterator(MAP_FAMILY);
    assertTrue(cells.hasNext());
  ...
  public void testMapAsIterable() throws IOException {
    ...
    final FijiRowData row1 = getRowData(mTable, mReader, eid, dataRequest);
    final List<FijiCell<Integer>> cells = Lists.newArrayList(row1.<Integer>asIterable(MAP_FAMILY));
  ...
  public void testGroupAsIterable() throws IOException {
    ...
    assertTrue(data.containsColumn(FAMILY));
    assertTrue(data.containsColumn(FAMILY, QUALIFIER_0));
    final List<FijiCell<CharSequence>> cells =
  ...
  public void testEmptyResult() throws IOException {
    ...
    final FijiDataRequest dataRequest = FijiDataRequest.create(FAMILY, QUALIFIER_1);
    ...
    final NavigableMap<Long, FijiCell<CharSequence>> cells = row1.getCells(FAMILY, QUALIFIER_1);
    assertTrue("getCells should return an empty map for empty rowdata.", cells.isEmpty());

    final Iterator<FijiCell<CharSequence>> iterator =  row1.iterator(FAMILY, QUALIFIER_1);
    ...
    final FijiCell<CharSequence> cell = row1.getMostRecentCell(FAMILY, QUALIFIER_1);
  ...
  public void testWriterSchemaWhenSpecificRecordClassNotFound() throws Exception {
    mFiji.createTable(FijiTableLayouts.getLayout(WRITER_SCHEMA_TEST));
    final FijiTable table = mFiji.openTable("writer_schema");
    try {
