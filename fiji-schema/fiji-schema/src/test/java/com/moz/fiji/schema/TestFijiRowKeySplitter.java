...
public class TestFijiRowKeySplitter {

  @Test
  public void testGetRowKeyResolution() throws IOException {
    assertEquals(2, FijiRowKeySplitter
        .getRowKeyResolution(FijiTableLayouts.getLayout(FijiTableLayouts.FORMATTED_RKF)));
    assertEquals(16, FijiRowKeySplitter
        .getRowKeyResolution(FijiTableLayouts.getLayout(FijiTableLayouts.FULL_FEATURED)));
  }
  ...
  public void testSplitKeysInclusive() {
    byte[][] splitKeys = FijiRowKeySplitter.get().getSplitKeys(256, 1);
    assertThat(splitKeys.length, is(255));
  }

  @Test
  public void testSplitKeysSimple() {
    final byte[][] splitKeys = FijiRowKeySplitter.get().getSplitKeys(2, 1);
  ...
  public void testSplitKeysInvalidArgument() {
    try {
      FijiRowKeySplitter.get().getSplitKeys(257, 1);
    ...
    try {
      FijiRowKeySplitter.get().getSplitKeys(65537, 2);
      Assert.fail("Should be invalid!");
    } catch (IllegalArgumentException iae) {
