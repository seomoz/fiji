...
public class TestVersionInfo {
  ...
  public void testGetClusterDataVersion() throws Exception {
    final FijiSystemTable systemTable = createMock(FijiSystemTable.class);
    ...
    final Fiji fiji = createMock(Fiji.class);
    expect(fiji.getSystemTable()).andReturn(systemTable).anyTimes();

    replay(systemTable);
    replay(fiji);
    assertEquals(ProtocolVersion.parse("system-1.1"), VersionInfo.getClusterDataVersion(fiji));
    verify(systemTable);
    verify(fiji);
  }
  ...
  public void testValidateVersion() throws Exception {
    ...
    VersionInfo.validateVersion(fiji);
  ...
  public void testValidateVersionFail() throws Exception {
    final FijiSystemTable systemTable = createMock(FijiSystemTable.class);
    expect(systemTable.getDataVersion()).andReturn(ProtocolVersion.parse("fiji-0.9")).anyTimes();
    ...
    } catch (IncompatibleFijiVersionException ikve) {
      assertTrue(ikve.getMessage(), Pattern.matches(
          "Data format of Fiji instance \\(fiji-0\\.9\\) cannot operate "
  ...
  @Test
  public void testFijiVsSystemProtocol() {
    ...
    final ProtocolVersion clusterVersion = ProtocolVersion.parse("fiji-1.0");

    assertTrue(VersionInfo.areInstanceVersionsCompatible(clientVersion, clusterVersion));
  }

  @Test
  public void testOldFijiRefusesToReadNewSystemProtocol() {
    ...
    final ProtocolVersion clientVersion = ProtocolVersion.parse("fiji-1.0");
  ...
  @Test
  public void testFijiVsNewerSystemProtocol() {
    ...
    // fiji-1.0 => system-1.0, and all system-1.x versions should be compatible.
