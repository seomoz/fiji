 ...
 */
public class TestStrictValidation extends FijiClientTest {
  ...
  public static final String LAYOUT_UNION_WRITER =
      "com.moz.fiji/schema/layout/TestStrictValidation.union-writer.json";

  public static final String LAYOUT_ENUM =
      "com.moz.fiji/schema/layout/TestStrictValidation.enum.json";
  ...
  public void testUnionWriterSchema() throws Exception {
    ...
    fiji.createTable(FijiTableLayouts.getLayout(LAYOUT_UNION_WRITER));
    final FijiTable table = fiji.openTable(TABLE_NAME);
    try {
      final EntityId eid = table.getEntityId("row");
      final FijiTableWriter writer = table.getWriterFactory().openTableWriter();
      try {
        ...
        final List<AvroSchema> expectedIds = Lists.newArrayList(
            AvroSchema.newBuilder()
                .setUid(fiji.getSchemaTable().getOrCreateSchemaId(unionNullString))
                .build());

        final List<AvroSchema> writerSchemaIds =
            table.getLayout().getCellSchema(FijiColumnName.create("info:user")).getWriters();
        Assert.assertEquals(expectedIds, writerSchemaIds);

        final List<AvroSchema> writtenSchemaIds =
            table.getLayout().getCellSchema(FijiColumnName.create("info:user")).getWritten();
  ...
  public void testEnumSchema() throws Exception {
    final Fiji fiji = getFiji();
    fiji.createTable(FijiTableLayouts.getLayout(LAYOUT_ENUM));
    ...
    try {
      ...
      final TableLayoutDesc updateWithReader =
          new TableLayoutBuilder(table.getLayout().getDesc(), fiji)
              .withReader(FijiColumnName.create("info",  "gender"), readerEnum)
              .build();
      LOG.debug("Applying update with enum reader: %s", updateWithReader);
      fiji.modifyTableLayout(updateWithReader);

      final TableLayoutDesc updateWithWriter=
          new TableLayoutBuilder(table.getLayout().getDesc(), fiji)
              .withWriter(FijiColumnName.create("info",  "gender"), writerEnum)
              .build();
      LOG.debug("Applying update with enum writer: %s", updateWithWriter);
      try {
        fiji.modifyTableLayout(updateWithWriter);
        Assert.fail();
