...
public class TestNotesTool extends FijiClientTest {
  ...
  public void setup() throws IOException {
    getFiji().createTable(FijiTableLayouts.getLayout(FijiTableLayouts.USER_TABLE));
  ...
  private FijiURI getInfoNameURI() {
    return FijiURI.newBuilder(mTable.getURI())
        .withColumnNames(Lists.newArrayList(FijiColumnName.create("info:name"))).build();
  }
  ...
  public void testValidateFlags() throws Exception {
    assertFailure("--target=fiji://.env/*^%/table");
    assertTrue(mToolOutputStr.startsWith("Invalid Fiji URI:"));

    assertFailure("--target=fiji://.env/default");
    assertTrue(mToolOutputStr.startsWith("--target URI must specify at least a table, got:"));

    assertFailure("--target=fiji://.env/default/table/info:name,info:email");
    assertEquals("--target URI must include one or zero columns.\n", mToolOutputStr);

    assertFailure("--target=fiji://.env/default/table/info:name", "--do=fake");
    assertEquals("Invalid --do command: 'fake'. Valid commands are set, remove, get\n",
        mToolOutputStr);

    assertFailure("--target=fiji://.env/default/table/info:name", "--do=get", "--in-family=info",
        "--key=key");
