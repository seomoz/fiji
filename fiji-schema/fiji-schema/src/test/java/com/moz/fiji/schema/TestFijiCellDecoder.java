...
/** Tests for Fiji cell decoders. */
public class TestFijiCellDecoder extends FijiClientTest {
  @Test
  public void testDecodeCounter() throws IOException {
    ...
    final FijiCellDecoder<Long> decoder = SpecificCellDecoderFactory.get().create(cellSpec);
  ...
  private void testDecodeAvroSchema(SchemaStorage storage) throws IOException {
    ...
    {
      final FijiCellDecoder<Node> decoder = SpecificCellDecoderFactory.get().create(cellSpec);
    ...
    {
      final FijiCellDecoder<GenericRecord> decoder =
