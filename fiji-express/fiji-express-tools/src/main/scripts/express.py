...
# The express script provides tools for running Express jobs and interacting with FijiExpress.
...
EXPRESS_TOOL = "com.moz.fiji.express.flow.ExpressTool"
...
class ExpressTool(object):
    """Wrapper for the FijiExpress installation."""
    ...
    def __init__(
        ...
        """Initializes a new FijiExpress wrapper.
        ...
        self._fiji = FijiTool(env=self._env)
    ...
    def _list_classpath_entries(self):
        ...
        yield from self.fiji.classpath
...
def make_arg_parser():
    ...
    parser = argparse.ArgumentParser(
        description="FijiExpress command-line interface.",
        epilog=ENV_VAR_HELP,
    ...
    classpath_parser = subparsers.add_parser(
        "classpath",
        help="Prints the FijiExpress classpath on the standard output.",
        formatter_class=text_formatter,
    )
    jar_parser = subparsers.add_parser(
        "jar",
        help="Runs an arbitrary Java program with FijiExpress libraries on the classpath.",
        formatter_class=text_formatter,
    )
    job_parser = subparsers.add_parser(
        "job",
        help="Submits a FijiExpress job.",
        formatter_class=text_formatter,
    )
    shell_parser = subparsers.add_parser(
        "shell",
        help="Starts the FijiExpress console shell.",
        formatter_class=text_formatter,
    )
    schema_shell_parser = subparsers.add_parser(
        "schema-shell",
        help="Starts the FijiSchema DDL shell with FijiExpress extensions loaded.",
        formatter_class=text_formatter,
    )
...
class ExpressCLI(object):
    ...
    def jar(self):
        ...
        cmd = [
            "java",
            # This property is only needed in fiji-schema v1.1 :
            "-Dcom.moz.fiji.schema.impl.AvroCellEncoder.SCHEMA_VALIDATION=DISABLED",
        ] + java_opts + [
    ...
    def job(self):
        ...
        cmd = [
            "java",
            # Property needed for fiji-schema v1.1:
            "-Dcom.moz.fiji.schema.impl.AvroCellEncoder.SCHEMA_VALIDATION=DISABLED",
            "-classpath", ":".join(classpath),
    ...
    def schema_shell(self):
        ...
        assert os.path.isdir(schema_shell_home), \
            ("Invalid home directory for FijiSchema shell: %r" % schema_shell_home)
        schema_shell_script = os.path.join(schema_shell_home, "bin", "fiji-schema-shell")
        assert os.path.isfile(schema_shell_script), \
            ("FijiSchema shell not found: %r" % schema_shell_script)
        ...
        # Relevant for FijiSchema 1.1 only and will be removed in Express 3.0:
        java_opts += " -Dcom.moz.fiji.schema.impl.AvroCellEncoder.SCHEMA_VALIDATION=DISABLED"
