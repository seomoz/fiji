...
/**
 * Adds extra repl-specific functionality to FijiPipe, allowing the user to build
...
@Inheritance.Sealed
class FijiPipeTool(private[express] val pipe: Pipe) {
  /**
   ...
   */
  private[express] def getJob(args: Args): Job = new FijiJob(args) {
    /**
     ...
     * but adds options specific to FijiExpress, including adding a jar containing compiled REPL
     ...
     * but clears the implicit flow definition defined in [[com.moz.fiji.express.repl.Implicits]]
