...
import com.moz.fiji.express.flow.util.PipeRunner._

@RunWith(classOf[JUnitRunner])
class PackUnpackRecordSuite extends FijiSuite {

  test("Avro tuple converters should be in implicit scope of FijiJobs.") {
    class ImplicitJob(args: Args) extends FijiJob(args) {
  ...
  import com.moz.fiji.express.Implicits._
  ...
  test("A FijiPipe can pack fields into a specific Avro record.") {
  ...
  test("A FijiPipe can pack fields into a generic Avro record.") {
  ...
  test("A FijiPipe can unpack a specific Avro record into fields.") {
  ...
  test("A FijiPipe can unpack a generic Avro record into fields.") {
    val pipe = new Pipe("unpack generic record")
