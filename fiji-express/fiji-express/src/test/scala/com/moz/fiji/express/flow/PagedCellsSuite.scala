 ...
 * in a Fiji table, and then concatenates those strings into a single word.
 *
 ...
 *     Tsv file.
 */
class WordConcatJob(args: Args) extends FijiJob(args) {
 ...
 *     Tsv file.
 */
class WordCountFlatMapJob(args: Args) extends FijiJob(args) {
  // Setup input to bind values from the "family:column1" column to the symbol 'word.
  FijiInput.builder
      .withTableURI(args("input"))
      .withColumnSpecs(QualifiedColumnInputSpec.builder
...
class PagedCellsSuite extends FijiSuite {
  ...
  test("a word-concat job that reads from a Fiji table is run using Scalding's local mode") {
  ...
  test("a word-count job that reads from a Fiji table is run using Scalding's local mode") {
    // Create test Fiji table.
    val uri: String =
        ResourceUtil.doAndRelease(makeTestFijiTable(simpleLayout)) { table: FijiTable =>
          table.getURI.toString
