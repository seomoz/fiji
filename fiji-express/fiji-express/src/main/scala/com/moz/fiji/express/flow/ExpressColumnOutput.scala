...
/**
 * A class that holds information identifying a single qualified column in a Fiji table. An
 * object of this class is required for writing data via the TypeSafe API.
 *
 * The sink implementations for [[com.moz.fiji.express.flow.framework.TypedFijiScheme]] and
 * [[com.moz.fiji.express.flow.framework.TypedLocalFijiScheme]] expect the
 ...
 *
 *   //Writes values from a pre-existing typed pipe to two columns in fiji table.
 ...
 *      List(col1, col2)
 *   }.write(FijiOutput.typedSinkForTable("fiji://.env/mytable"))
 *
 * }}}
 *
...
final class ExpressColumnOutput[T] private (
  ...
  /**
   * The [[FijiColumnName]] for the column specified by this object.
   *
   * @return The FijiColumn name identifying the column being written to.
   */
  def columnName: FijiColumnName = FijiColumnName.create(family, qualifier)
}
