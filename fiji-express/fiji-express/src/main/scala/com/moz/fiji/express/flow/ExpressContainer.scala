...
abstract class ExpressContainer(
  ...
  // the same as programming in a FijiJob.
...
object ExpressContainer {
   ...
   * @param mode an implicit Mode object passed in from the FijiJob.
   * @param flowDef an implicit FlowDef object passed in from the FijiJob.
...
trait ExpressContainerFactory[T] {
   ...
   * within a FijiJob. This will throw an exception if the resulting container does not implement
   * the interface T.
   *
   * @param key the key in the Args object that points to the configuration of the container.
   * @param fijiArgs Implicit Args object passed from a FijiJob.
   * @param mode Implicit Mode object passed from a FijiJob.
   * @param flowDef Implicit FlowDef object passed from a FijiJob.
  ...
  def apply(
    ...
    val container: ExpressContainer = ExpressContainer(fijiArgs(key))
    container match {
