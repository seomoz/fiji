 ...
 * read from a Fiji table to Scala values (including our Express-specific generic API Avro values)
 * for use in FijiExpress, and vice versa for writing back to a Fiji table.
 *
 * When reading from Fiji, using the specific API, use the method `decodeSpecificFromJava`.
 * When reading from Fiji, using the generic API, use the method `decodeGenericFromJava`.
 * When writing to Fiji, use the method `encodeToJava`.
 *
