...
import com.moz.fiji.express.flow.framework.hfile.HFileFlowStepStrategy
import com.moz.fiji.express.flow.framework.hfile.HFileFijiTap
import com.moz.fiji.express.flow.framework.serialization.FijiKryoInstantiator
import com.moz.fiji.express.flow.histogram.HistogramConfig
import com.moz.fiji.express.flow.histogram.TupleProfiling
import com.moz.fiji.express.flow.util.AvroTupleConversions
import com.moz.fiji.express.flow.util.PipeConversions
import com.moz.fiji.express.flow.util.ResourcesShutdown
import com.moz.fiji.schema.Fiji
import com.moz.fiji.schema.FijiURI

/**
 * FijiJob is FijiExpress's extension of Scalding's `Job`, and users should extend it when writing
 * their own jobs in FijiExpress.  It provides extra conversions that Express needs for FijiPipes.
 *
 * @param args to the job. These get parsed in from the command line by Scalding.  Within your own
 *     FijiJob, `args("input")` will evaluate to "SomeFile.txt" if your command line contained the
 *     argument `--input SomeFile.txt`
 */
@ApiAudience.Public
@ApiStability.Stable
@Inheritance.Extensible
class FijiJob(args: Args)
    extends Job(args)
  ...
  /** Implicit fijiArgs def for use by ExpressContainerFactory. */
  implicit final protected def fijiArgs: Args = args
  ...
  override def buildFlow: Flow[_] = {
    ...
    // Validate that the Fiji parts of the sources (tables, columns) are valid and exist.
    taps.foreach {
      case tap: FijiTap => tap.validate(conf)
      case tap: HFileFijiTap => tap.validate(conf)
      case tap: LocalFijiTap => {
        val properties: Properties = new Properties()
        properties.putAll(HadoopUtil.createProperties(conf))
        tap.validate(properties)
      }
      case _ => // No Fiji parts to verify.
    }
  ...
  private def checkpointHFileSink(): Unit = {
    ...
    val hfileSinks = sinks.asScala.collect { case (name, _: HFileFijiTap) => name }.toSet
  ...
  override def config: Map[AnyRef, AnyRef] = {
    ...
    // disable schema validation. This system property is only useful for FijiSchema v1.1. In newer
    // versions of FijiSchema, this property has no effect.
    val disableValidation = " -Dcom.moz.fiji.schema.impl.AvroCellEncoder.SCHEMA_VALIDATION=DISABLED"
  ...
  /**
   * Record the history of this job into all relevant Fiji instances. This includes all Fiji
  ...
  private def recordJobHistory(startTime: Long, endTime: Long, jobSuccess: Boolean) {
    def maybeGetTableUri(t: Tap[_, _, _]): Option[String] = {
      t match {
        case kt: FijiTap => Some(kt.tableUri)
        case lkt: LocalFijiTap => Some(lkt.tableUri)
        case hfkt: HFileFijiTap => Some(hfkt.tableUri)
        case _ => None
      }
    }
    ...
    val extendedInfo: Map[String, String] = args.list(FijiJob.extendedInfoArgsKey).map {
      s: String => {
    ...
    val instanceUris: Set[FijiURI] =
        (flowDef.getSources.asScala.toList ++ flowDef.getSinks.asScala)
        .map { pair: (String, Tap[_, _, _]) => maybeGetTableUri(pair._2) }
        .flatten
        .toSet
        .map { uriString: String => FijiURI.newBuilder(uriString).build() }

    instanceUris.foreach { uri: FijiURI =>
      val fiji: Fiji = if (conf.isDefined) {
        Fiji.Factory.open(uri, conf.get)
      } else {
        Fiji.Factory.open(uri)
      }
      try {
        recordJobHistory(
            startTime,
  ...
  /**
   * Record the history of this job into the given Fiji instance.
   *
   * @param startTime in milliseconds since the epoch at which the job started.
   * @param endTime in milliseconds since the epoch at which the job ended.
   ...
   * @param fiji instance into which to record the job history.
...
/** Companion object to FijiJob. */
object FijiJob {
  /** Logger for the FijiJob class. */
  val logger: Logger = LoggerFactory.getLogger(classOf[FijiJob])
  ...
  /** Arg used by FijiExpress to determine whether to add the classpath to the dist. cache. */
  val addClasspathArg: String = "add-classpath-to-dcache"

  /** Arg used by FijiExpress, whose values are comma-separated jars to add to the dist.cache. */
  val tmpjarsArg: String = "tmpjars"

  /** Mapreduce configuration property for tmpjars. */
  val tmpjarsConfigProperty: String = "tmpjars"

  /** Prefix for riemann events related to fiji-express. */
  val RIEMANN_EVENT_PREFIX = "fiji.express"
