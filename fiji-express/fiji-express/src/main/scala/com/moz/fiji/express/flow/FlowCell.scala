...
/**
 * A container for data from a Fiji table. Contains some datum tagged with a column family, column
 * qualifier, and version. Flow cells are provided when requesting a Fiji table column or column
 * family.
 *
...
object FlowCell {
   ...
   * Fiji table along with the data itself.
   *
   * @tparam T is the type of the datum that this cell contains.
   * @param cell from a Fiji table produced by the Java API.
   * @return a FlowCell for use in FijiExpress containing the same family, qualifier, version, and
   *     datum as the provided FijiCell.
   */
  private[fiji] def apply[T](cell: FijiCell[T]): FlowCell[T] = {
    new FlowCell[T](
        cell.getFamily,
        cell.getQualifier,
   ...
   * [[com.moz.fiji.express.flow.FlowCell]]s by value.
   *
   * @tparam T is the type of the datum in the [[com.moz.fiji.express.flow.FlowCell]].
  ...
  /**
   * Provides an implementation of the `scala.Ordering` trait that sorts
   * [[com.moz.fiji.express.flow.FlowCell]]s by version.
   *
   * @tparam T is the type of the datum in the [[com.moz.fiji.express.flow.FlowCell]].
  ...
  /**
   * Provides an implementation of the `scala.Ordering` trait that sorts
   * [[com.moz.fiji.express.flow.FlowCell]]s first by the cell's family and then by it's qualifier.
   *
   * @tparam T is the type of the datum in the [[com.moz.fiji.express.flow.FlowCell]].
   ...
   * the default order for scan results from a Fiji table.
   *
