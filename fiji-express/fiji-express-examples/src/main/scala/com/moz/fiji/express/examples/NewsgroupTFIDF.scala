 ...
 * which is evaluated in [[com.moz.fiji.express.examples.NewsgroupClassifier]].
 *
 * Example usage:
 *    express job /path/to/this/jar com.moz.fiji.express.examples.NewsgroupTFIDF \
 ...
 *
 * @param args to the job. `--table fiji://path/to/myTable` specifies the table to run on.
 *     `--out-file file://path/to/outputFile` specifies the path to the output file,
 *     which will be a Tsv.
 */
class NewsgroupTFIDF(args: Args) extends FijiJob(args) {
