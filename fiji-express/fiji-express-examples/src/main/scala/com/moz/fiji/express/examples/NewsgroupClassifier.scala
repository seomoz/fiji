 ...
 * [[com.moz.fiji.express.examples.NewsgroupTFIDF]] on each post in the testing set, compares the result
 * of the classifier with the known label, and reports the overall accuracy of the classifier.
 *
 * Example usage:
 *    express job /path/to/this/jar com.moz.fiji.express.examples.NewsgroupClassifier \
 ...
 *
 * --weights-file should be the same as --out-file of [[com.moz.fiji.express.examples.NewsgroupTFIDF]]
 *
 * @param args to the job. `--table fiji://path/to/myTable` specifies the table to run on (where
 ...
 */
class NewsgroupClassifier(args: Args) extends FijiJob(args) {
