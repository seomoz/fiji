...
class Action(cli.Action):
    """CLI action that applies within a Fiji workspace."""

    def __init__(self, workspace, **kwargs):
        """Initializes a CLI action on the specified Fiji workspace.

        Args:
            workspace: Fiji workspace this action applies to.
        """
    ...
    @property
    def workspace(self):
        """Returns: the Fiji workspace the action is being applied to."""
...
class List(Action):
    ...
    USAGE = """
        ...
        |
        |Usage:
        |    fiji-build list [--filter-kind=<kind>] <target1> ...
        |
        |Example:
        ...
        |    $ fiji-build list --filter-kind=java_binary
        |
        |- List the direct dependencies of "//com/moz/fiji/schema:fiji-schema":
        |    $ fiji-build list --deps=upstream //com/moz/fiji/schema:fiji-schema
        |
        |- List the build targets that depend on "//com/moz/fiji/schema:fiji-schema":
        |    $ fiji-build list --deps=downstream-transitive //com/moz/fiji/schema:fiji-schema
        |
        |Note:
...
class Build(Action):
    ...
    USAGE = """
        ...
        |
        |Usage:
        |  fiji-build build [--targets=]<target> ...
        """
    ...
    def register_flags(self):
        ...
        self.flags.add_boolean(
            ...
            help=("When set logs from tests will be written to files separate from the main log.\n"
                  "When unset, logs from tests will be appended to the fiji-build main log file."),
        )
        self.flags.add_string(
...
class Run(Build):
    ...
    USAGE = """
        ...
        |
        |Usage:
        |    fiji-build run [<target-or-id> <target-or-id> ...]
        """
...
class GeneratePom(Build):
    ...
    USAGE = """
        ...
        |
        |Usage:
        |    fiji-build generate-pom [<target> <target> ...]
        """
...
class Checkstyle(Build):
    ...
    USAGE = """
        ...
        |
        |Usage:
        |    fiji-build checkstyle [<target> <target> ...]
        """
...
class Scalastyle(Build):
    ...
    USAGE = """
        ...
        |
        |Usage:
        |    fiji-build scalastyle [<target> <target> ...]
        """
...
<classpath>
  ...
  <classpathentry combineaccessrules="false" kind="src" path="/fiji-delegation"/>
...
USAGE = """\
Usage:
    fiji-build [--do=]command [--flags ...] arguments...
...
Command-specific help may be obtained with:
    fiji-build <command> --help

Wiki documentation:
    https://wiki.wibidata.com/display/ENG/fiji-build+v2
"""
    ...
    def register_flags(self):
        ...
        self.flags.add_string(
            ...
            help=('Workspace base directory.\n'
                  ...
                  'None means use the enclosing Fiji workspace.'),
        )
        self.flags.add_string(
...
def main(args):
    ...
    """
    tool = FijiBuildTool(parent_flags=base.FLAGS)
    return tool(args)
