...
import com.moz.fiji.express.music.avro.SongMetadata

/**
 * Imports metadata about songs into a Fiji table.
 *
 * This importer expects two command line arguments: `--table-uri` and `--input`. The
 * argument `--table-uri` should be set to the Fiji URI of a songs table that the import will
 ...
 */
class SongMetadataImporter(args: Args) extends FijiJob(args) {
  /**
  ...
  // 5. Writes the Avro records to the column "info:metadata" in a row for the song in a Fiji
  //    table.
  TextLine(args("input"))
