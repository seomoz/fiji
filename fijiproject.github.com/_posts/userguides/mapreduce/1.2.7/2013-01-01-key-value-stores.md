...
KeyValueStores are used to provide MapReduce programs and other operators processing Fiji datasets with the ability to join datasets. One data set can be specified as a key-value store using the KeyValueStore API. The program can use a KeyValueStoreReader to look up values associated with keys. These keys are often driven by records of a dataset being processed by MapReduce.

This can be used, for example, to provide a [`FijiProducer`]({{site.api_mr_1_2_7}}/produce/FijiProducer.html) with the means to apply the results of a trained machine learning model to the main data set. The output of a machine learning model might be expressed as (key, value) pairs stored in files in HDFS, or in a secondary Fiji table. For each user in a users table, you may want to compute a new recommendation for the user by applying the model to the information in the user's row. A value in the user's row may be a key into some arbitrary key-value store representing the model; the returned value is the recommendation.
...
The main classes in the KeyValueStore API are `org.fiji.mapreduce.kvstore.KeyValueStore` and `org.fiji.mapreduce.kvstore.KeyValueStoreReader`.

A [`KeyValueStore`]({{site.api_mr_1_2_7}}/kvstore/KeyValueStore.html) specifies all the resources needed to surface key-value pairs from some backing store. This may be defined on files, a Fiji table, or some other resource like a different NoSQL database.

Several KeyValueStore implementations are made available in the `org.fiji.mapreduce.kvstore.lib` package that cover common use cases for file- or Fiji-backed datasets. You could write your own [`KeyValueStore`]({{site.api_mr_1_2_7}}/kvstore/KeyValueStore.html) implementation that accesses a foreign system (e.g., a Redis database).

A [`KeyValueStore`]({{site.api_mr_1_2_7}}/kvstore/KeyValueStore.html) class specifies how to read data into the store. For example, [`TextFileKeyValueStore`]({{site.api_mr_1_2_7}}/kvstore/lib/TextFileKeyValueStore.html) expects to parse delimited text files, while [`SeqFileKeyValueStore`]({{site.api_mr_1_2_7}}/kvstore/lib/SeqFileKeyValueStore.html) reads SequenceFiles. Both of these stores are configured with an HDFS path to read from. A [`FijiTableKeyValueStore`]({{site.api_mr_1_2_7}}/kvstore/lib/FijiTableKeyValueStore.html) requires different configuration.
...
Your MapReduce task class (FijiMapper, FijiReducer, FijiProducer, FijiGatherer, etc) can implement [`KeyValueStoreClient`]({{site.api_mr_1_2_7}}/kvstore/KeyValueStoreClient.html) to help specify to a Fiji job builder (e.g., [`FijiGatherJobBuilder`]({{site.api_mr_1_2_7}}/gather/FijiGatherJobBuilder.html)) that it requires stores. This will require that you implement a method named `getRequiredStores()` that returns a mapping from names to KeyValueStores. These are the default _bindings_ for KeyValueStores.
...
Within the `FijiGatherer.gather()` method, the [`FijiContext`]({{site.api_mr_1_2_7}}/FijiContext.html) object provided as an argument will provide you with access to KeyValueStoreReaders. Since you may need multiple KeyValueStores, you can refer to each by the name you bound it to in `getRequiredStores()`.

Since each call to `gather()` is likely to require the same stores, it would be high-overhead to open and close a KeyValueStoreReader in every call. The FijiContext itself uses a class called [`KeyValueStoreReaderFactory`]({{site.api_mr_1_2_7}}/kvstore/KeyValueStoreReaderFactory.html), which is responsible for instantiating KeyValueStores from the `Configuration` associated with the job, and maintaining a pool of lazily-opened KeyValueStoreReader instances organized by name binding. The gatherer itself does not close individual KeyValueStoreReader instances. The KeyValueStoreReaderFactory will close all of them as the task is being torn down.
...
{% highlight xml %}
<stores>
  <store name="mystore" class="org.fiji.mapreduce.kvstore.lib.TextFileKeyValueStore">
    <configuration>
      <property>
        <name>paths</name>
...
We could write a FijiProducer that read from the `info:recently_watched` column and produce a field named `info:recommended_movie` as follows:
...
{% highlight java %}
    ...
    import org.fiji.mapreduce.*;
    import org.fiji.mapreduce.kvstore.*;
    import org.fiji.mapreduce.kvstore.lib.*;
    import org.fiji.schema.*;

    /**
     * Produce a movie recommendation based on the theory that if they liked
     * the first one, they should watch the sequel.
     */
    public class MoveRecProducer extends FijiProducer {
      ...
      @Override FijiDataRequest getDataRequest() {
        return FijiDataRequest.create("info", "recently_watched");
      }
    ...
    @Override void produce(FijiRowData input, ProducerContext context)
        throws IOException {
...
Several implementations of KeyValueStore are available in the `org.fiji.mapreduce.kvstore.lib` package:
...
You can also access a specific column of a Fiji table by the row's entityId using the [`FijiTableKeyValueStore`]({{site.api_mr_1_2_7}}/kvstore/lib/FijiTableKeyValueStore.html).
