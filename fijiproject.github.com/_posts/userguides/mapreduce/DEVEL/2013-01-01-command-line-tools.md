...
*   `--start-row=` and `--limit-row=`:
    ...
    Start and limit rows are expressed in the same way as `--entity-id` for [`fiji get`]({{site.userguide_schema_devel}}/tool-reference/#ref.get).
...
*   `--importer=java.package.BulkImporterClassName`: specifies the [`FijiBulkImporter`]({{site.api_mr_devel}}/bulkimport/FijiBulkImporter.html) class to use.

Producers must specify the name of the class providing the producers logic:

*   `--producer=java.package.ProducerClassName`: specifies the [`FijiProducer`]({{site.api_mr_devel}}/produce/FijiProducer.html) class to use.

Gatherer must specify the name of the class providing the gathering logic, and optionally a reducing logic:

*   `--gatherer=java.package.GathererClassName`: specifies the [`FijiGatherer`]({{site.api_mr_devel}}/gather/FijiGatherer.html) class to use.
