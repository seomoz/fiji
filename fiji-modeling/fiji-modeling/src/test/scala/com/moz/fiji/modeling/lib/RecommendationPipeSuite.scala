...
package com.moz.fiji.modeling.lib
...
class RecommendationPipeSuite extends FijiSuite {
  private val logger: Logger = LoggerFactory.getLogger(classOf[RecommendationPipeSuite])
  val testLayoutDesc: TableLayoutDesc = FijiTableLayouts.getLayout(FijiTableLayouts.SIMPLE)
  ...
  // The data consists of a Fiji table with the following 3 orders:
  // {milk, bread, coke}
  // {milk, bread, butter, flour}
  // {butter, flour}
  val fiji: Fiji = new InstanceBuilder("default")
  ...
  val inputUri: FijiURI = doAndRelease(fiji.openTable("OrdersTable")) {
    table: FijiTable => table.getURI
  }
  ...
  test("Prepare Itemsets Job runs properly") {
    class PrepareItemsetsJob(args: Args) extends FijiModelingJob(args) {
      FijiInput.builder
  ...
  test("Joining with Group Count works properly") {
    class JoinWithGroupCountJob(args: Args) extends FijiModelingJob(args) {
      FijiInput.builder
          .withTableURI(args("input"))
          .withColumns("family:column" -> 'slice)
          .build
  ...
  test("Joining with Count works properly") {
    class JoinWithCountJob(args: Args) extends FijiModelingJob(args) {
      FijiInput.builder
          .withTableURI(args("input"))
          .withColumns("family:column" -> 'slice)
          .build
  ...
  test("Filter by Support works with Constant Normalizer") {
    class FrequentItemsetFinderJob(args: Args) extends FijiModelingJob(args) {
      FijiInput.builder
          .withTableURI(args("input"))
          .withColumns("family:column" -> 'slice)
          .build
  ...
  test("findSimilarItemsUsingLocalitySensitiveHashing detects 90% similar items with a 70%" +
    ...
    class FindSimilarItemsNoFalseNegativesJob(args : Args) extends FijiModelingJob(args) {
      TextLine(args("input"))
          .read
  ...
  test("findSimilarItemsUsingLocalitySensitiveHashing does not detect 40% similar items with a " +
    ...
    class FindSimilarItemsNoFalsePositivesJob(args : Args) extends FijiModelingJob(args) {
      TextLine(args("input"))
          .read
          .map(('offset, 'line) -> ('productId, 'userIds)) {
            lineTuple : (Int, String) => {
  ...
  private def testItemItemSimilarity(
    ...
    def validateSimilarity(outputBuffer: mutable.Buffer[(Long, Long, Double)]): Unit = {
      ...
      pairs2scores.keys.foreach{ key: (Long, Long) =>
        val fijiScore = pairs2scores(key)
        val expectedScore = expectedSimilarities(key)
        if (!floatEq(fijiScore, expectedScore)) {
    ...
    class SimilarityCalculator(args: Args) extends FijiModelingJob(args) {
  ...
  // Assumes every row in the Fiji table above represents a user.
  // More exciting Jaccard test.
  test("More abstract Jaccard distance test works.") {

    class JaccardDistanceCalculator(args: Args) extends FijiModelingJob(args) {
      TextLine(args("input"))
        .read
  ...
  test("Counting tuples on pipe in parallel works correctly") {
    ...
    class TupleCounter(args: Args) extends FijiModelingJob(args) {
      TextLine(args("input"))
          .read
          .count('total)
          .write(TextLine(args("output")))
