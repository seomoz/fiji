...
import com.moz.fiji.modeling.examples.ItemItemCF.avro.AvroItemSimilarity
import com.moz.fiji.modeling.examples.ItemItemCF.avro.AvroSortedSimilarItems

/**
 * Contains common functionality for different phases in the item-item CF flow.
 */
abstract class ItemItemJob(args: Args) extends FijiJob(args) {
  ...
  def createUserRatingsPipe(specificUser: Option[Long] = None): Pipe = {
    ...
    val userRatingsPipe: Pipe = FijiInput.builder
  ...
  def createMostSimilarItemsPipe(specificItems: Option[Set[Long]] = None): Pipe = {
    ...
    val mostSimilarPipe = FijiInput.builder
